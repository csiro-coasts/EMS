 ...
 *
 *  File: model/lib/grid/lsq/svd.c
 *  
 *  Description: Singular value decomposition
 *
 *  Least squares fitting via singular value decomposition
 *
 *  Provides SVD for dense matrices ported from EISPACK (1972-1973).
 *  Provides least squares fitting via singular value decomposition;
 *  can take into account standard deviation of individual
 *  measurements.
 *
 *  Copyright:
 ...
 *
 *  $Id: svd.c 5839 2018-06-28 03:38:07Z riz008 $
 *
 */
...
static void sortvector(int n, double* v, int* pos)
{
    ...
    iv = (indexedvalue *)alloc_1d(n, sizeof(indexedvalue));
...
void svd(double** A, int n, int m, double* w, double** V)
{
    ...
    for (k = n - 1; k >= 0; k--) {
        ...
        while (1) {
            ...
            its++;
            if (its > SVD_NMAX)
	      grid_quit("svd(): no convergence in %d iterations\n", SVD_NMAX);
...
{
    int*      pos = i_alloc_1d(n);
    double*  wold = d_alloc_1d(n);
    double** aold = d_alloc_2d(n, m);
    double** vold = d_alloc_2d(n, n);
    double wmax;
    int i, j;
...
{
    double** V = d_alloc_2d(ni, ni + nj);  /* V and B at once */
    double** B = &V[ni];
...
void svd_lsq_B(double** A, int ni, int nj, double* z, double* std, double** b, double* sol)
{
    double** V = d_alloc_2d(ni, ni + nj);  /* V and B at once */
    double** B = &V[ni];
    double *w = d_alloc_1d(ni);
    int nijmin = (ni < nj) ? ni : nj;
    int i, j, ii;
