...
/* Wrapper to call findcircumcentre() externally. Added by MH, 11/9/2017. */
void circumcentre(double *p1, double *p2, double *p3, double *x, double *y)
{
  struct mesh m;
  struct behavior b;
  REAL torg[2];
  ...
  b.noexact = 0;
  torg[0] = (REAL)p1[0]; torg[1] = (REAL)p1[1];
  tdest[0] = (REAL)p2[0]; tdest[1] = (REAL)p2[1];
  tapex[0] = (REAL)p3[0]; tapex[1] = (REAL)p3[1];
 
  findcircumcenter(&m, &b, torg, tdest, tapex,
		   cc, &xi, &eta, &minedge);
  *x = (double)cc[0];
  *y = (double)cc[1];
}
