 ...
 *
 *  File: model/lib/grid/gridmap/gridmap.c
 *  
 *  Description:
 *  Calculates transformations between physical and index space within
 *  a numerical grid
 *
 *  Copyright:
 ...
 *
 *  $Id: gridmap.c 5839 2018-06-28 03:38:07Z riz008 $
 *
 */
...
static subgrid* subgrid_create(gridmap* gm, poly* pl, int i1, int i2, int j1, int j2)
{
  ...
  aftersearch:
    ...
    if (j > j2)
        grid_quit("subgrid_create(): boundary vertex not in the grid\n");
...
static void cut_boundary(poly* pl, double** gx, double** gy, int horiz, int index, int start, int end, poly** pl1, poly** pl2)
{
    ...
    if (horiz) {                /* horizontal cut */
         ...
         */
        for (i = start; i < end; ++i) {
            ii1 = poly_find_index(pl, gx[index][i], gy[index][i]);
            if (ii1 < 0)
                /*
             ...
             */
            tmp = poly_find_index(pl, gx[index][i + 1], gy[index][i + 1]);
            if ((tmp < 0 && poly_contains_point(pl, gx[index][i + 1], gy[index][i + 1])) || (tmp >= 0 && abs(tmp - ii1) > 1 && abs(tmp - ii1) < n - 1))
                break;
        }
         ...
         */
        for (i = i1 + 1; i <= end; ++i) {
            ii2 = poly_find_index(pl, gx[index][i], gy[index][i]);
            if (ii2 >= 0)
                /*
        ...
        *pl2 = poly_create();
         ...
         */
        for (i = i2; i > i1; --i)
            poly_add_point(*pl1, gx[index][i], gy[index][i]);
         ...
         */
        for (i = i1; i < i2; ++i)
            poly_add_point(*pl2, gx[index][i], gy[index][i]);

    } else {                    /* vertical cut */
        for (i = start; i < end; ++i) {
            ii1 = poly_find_index(pl, gx[i][index], gy[i][index]);
            if (ii1 < 0)
                continue;
            tmp = poly_find_index(pl, gx[i + 1][index], gy[i + 1][index]);
            if ((tmp < 0 && poly_contains_point(pl, gx[i + 1][index], gy[i + 1][index])) || (tmp >= 0 && abs(tmp - ii1) > 1 && abs(tmp - ii1) < n - 1))
                break;
        }
        ...
        for (i = i1 + 1; i <= end; ++i) {
            ii2 = poly_find_index(pl, gx[i][index], gy[i][index]);
            if (ii2 >= 0)
                break;
        }
        ...
        *pl2 = poly_create();
        ...
        for (i = ii1; i != ii2; i = (i + 1) % n)
            poly_add_point(*pl1, pl->x[i], pl->y[i]);
        for (i = i2; i > i1; --i)
            poly_add_point(*pl1, gx[i][index], gy[i][index]);

        for (i = ii2; i != ii1; i = (i + 1) % n)
            poly_add_point(*pl2, pl->x[i], pl->y[i]);
        for (i = i1; i < i2; ++i)
            poly_add_point(*pl2, gx[i][index], gy[i][index]);
        /*
         ...
         * points... Anyway, considering the way search in poly_contains_point()
...
static void subgrid_divide(subgrid* sg, subgrid** sg1, subgrid** sg2)
{
    ...
    if (pl1 == NULL || pl2 == NULL)
        grid_quit("dividesubgrid(): could not cut the boundary\n");
...
int gridmap_xy2ij(gridmap* gm, double x, double y, int* i, int* j)
{
     ...
     */
    sg = gm->trunk;
    if (!poly_contains_point(sg->bound, x, y))
        return 0;
    ...
    while (sg->half1 != NULL) {
        ...
        if (sg->half1->bound->n <= sg->half2->bound->n)
            sg = (poly_contains_point(sg->half1->bound, x, y)) ? sg->half1 : sg->half2;
        else
            sg = (poly_contains_point(sg->half2->bound, x, y)) ? sg->half2 : sg->half1;
    }
