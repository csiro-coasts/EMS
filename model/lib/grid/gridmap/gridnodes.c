 ...
 *
 *  File: model/lib/grid/gridmap/gridnodes.c
 *  
 *  Description:
 *  Handling of grid node arrays
 *
 *  Copyright:
 ...
 *
 *  $Id: gridnodes.c 5839 2018-06-28 03:38:07Z riz008 $
 *
 */
...
gridnodes* gridnodes_read(char* fname, NODETYPE type)
{
    ...
    if (strcasecmp(fname, "stdin") == 0 || strcmp(fname, "-") == 0)
      ...
      f= fopen(fname, "r");
      if (f == NULL)
	grid_quit("gridnodes_read: could not open file '%s'\n", fname);
    }
    gn->type = type;
    ...
    if (fgets(buf, BUFSIZE, f) == NULL)
        grid_quit("%s: empty file\n", fname);

    if (sscanf(buf, "## %d x %d", &gn->nx, &gn->ny) != 2)
        grid_quit("%s: could not read grid size: expected header in \"## %%d x %%d\" format\n", fname);

    if (gu_verbose)
        fprintf(stderr, "##   %d x %d grid\n", gn->nx, gn->ny);

    if (gn->nx < 1)
        grid_quit("gridnodes_read(): nx = %d: invalid grid size\n", gn->nx);
    if (gn->ny < 1)
        grid_quit("gridnodes_read(): ny = %d: invalid grid size\n", gn->ny);
    if ((double) gn->nx * (double) gn->ny > (double) INT_MAX)
        grid_quit("gridnodes_read(): grid size (%d x %d) is too big\n", gn->nx, gn->ny);

    if (type == NT_DD) {
        if (gn->nx % 2 == 0)
            grid_quit("gridnodes_read(): nx = %d must be odd for double density grid nodes\n", gn->nx);
        if (gn->ny % 2 == 0)
            grid_quit("gridnodes_read(): ny = %d must be odd for double density grid nodes\n", gn->ny);
    }

    /*
     * allocate storage 
     */
    gn->gx = d_alloc_2d(gn->nx, gn->ny);
    gn->gy = d_alloc_2d(gn->nx, gn->ny);
    ...
    for (j = 0, xx = gn->gx[0], yy = gn->gy[0], count = 0; j < gn->ny; ++j) {
        for (i = 0; i < gn->nx; ++i, ++xx, ++yy) {
            if (fgets(buf, BUFSIZE, f) == NULL)
                grid_quit("%s: could not read %d-th point (%d x %d points expected)\n", fname, j * gn->nx + i + 1, gn->nx, gn->ny);
...
gridnodes* gridnodes_create(int nx, int ny, NODETYPE type)
{
    ...
    gn->nx = nx;
    gn->ny = ny;
    gn->gx = d_alloc_2d(nx, ny);
    gn->gy = d_alloc_2d(nx, ny);
    gn->type = type;
    gn->validated = 0;
    gn->stats = NULL;
    gn->nextpoint = 0;
...
gridnodes* gridnodes_copy(gridnodes* old)
{
    ...
    new->ny = old->ny;
    new->type = old->type;
    new->gx = d_alloc_2d(old->nx, old->ny);
    new->gy = d_alloc_2d(old->nx, old->ny);
...
gridnodes* gridnodes_subgrid(gridnodes* gn, int imin, int imax, int jmin, int jmax)
{
    ...
    new->gx = d_alloc_2d(new->nx, new->ny);
    new->gy = d_alloc_2d(new->nx, new->ny);
...
gridnodes* gridnodes_transform(gridnodes* gn, NODETYPE type)
{
    ...
    if (gn->type == NT_DD) {
        if (type == NT_COR) {
            gn1->nx = gn->nx / 2 + 1;
            gn1->ny = gn->ny / 2 + 1;
            gn1->gx = d_alloc_2d(gn1->nx, gn1->ny);
            gn1->gy = d_alloc_2d(gn1->nx, gn1->ny);
        ...
        grid_quit("transforming nodes of type \"%s\" into \"%s\" is not supported\n", nodetype2str[gn->type], nodetype2str[type]);
    }
...
void gridnodes_applymask(gridnodes* gn, int** mask)
{
    if (gn->type == NT_DD) {
        ...
        grid_quit("gridnodes_applymask(): applying mask to nodes of type \"%s\" is not supported\n", nodetype2str[NT_COR]);
    else if (gn->type == NT_NONE)
        grid_quit("gridnodes_applymask(): nodes type not specified\n");
}
...
void gridnodes_write(gridnodes* gn, char* fname, COORDTYPE ctype)
{
    ...
    if (!strcasecmp(fname, "stdout")) {
      ...
      f = fopen(fname, "w");
      if (f == NULL)
	grid_quit("gridnodes_write: could not open file '%s'\n", fname);
    }
    fprintf(f, "## %d x %d\n", nx, ny);
...
int gridnodes_getnce1(gridnodes* gn)
{
    if (gn->type == NT_DD)
        ...
        grid_quit("gridnodes_getnce1(): node type not specified\n");
...
int gridnodes_getnce2(gridnodes* gn)
{
    if (gn->type == NT_DD)
        ...
        grid_quit("gridnodes_getnce2(): node type not specified\n");
