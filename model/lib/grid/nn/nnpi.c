 ...
 *
 *  $Id: nnpi.c 5859 2018-07-02 04:04:44Z her127 $
 *
 */
...
struct nnpi {
  ...
  hash_table_t* bad;             /* ids of vertices that require a special
				  * treatment */
  NN_RULE rule;
  int test_vertice;
};
...
static void nnpi_triangle_process(nnpi* nn, point* p, int i)
{
    ...
    for (j = 0; j < 3; ++j) {
        ...
        if (!circle_build(&cs[j], &d->points[v1], &d->points[v2], p, 1)) {
    ...
    for (j = 0; j < 3; ++j) {
        ...
        if (isnan(det)) {
             ...
             * recognised as very large in circle_build(..,1), then it
             * parameters are replaced by NaNs, which results in det
             * above being NaN. 
             * 
             ...
             * of each pair cancelling each other if the point moves slightly
             * off the edge.
             *
             * Each of the determinants corresponds to a triangle, and an
             * inifinite determinant corresponds to a triangle with one vertex
             * moved to infinity. "Subtracting" one triangle from another within
             * each pair yields a valid quadrilateral (in fact, a trapezoid).
             * The doubled area of these quadrilaterals is calculated in the
             * cycle over ii below.
             */
	      ...
	      nn->bad = ht_create_i2(HT_SIZE);
            ...
            if (v == NULL) {
                ...
                free(v);
                ht_delete(nn->bad, &key); // v is already free so no
...
static int compare_indexedpoints(const void* pp1, const void* pp2)
{
  ...
  if (points_onleftside(a, p0, b) == 1) {
    if ((points_onleftside(a, p0, p1) == 1) && 
	(points_onleftside(b, p0, p1) != 1)) {
      /*
    ...
    if ((points_onleftside(b, p0, p1) == 1) && 
	(points_onleftside(a, p0, p1) != 1)) {
      /*
...
static int _nnpi_calculate_weights(nnpi* nn, point* p)
{
    ...
    if (nn->rule == SIBSON) {
      ...
      grid_quit("_nnpi_calculate_weights: programming error");
...
void nnpi_interpolate_point(nnpi* nn, point* p)
{
	...
	for (i = 0; i < nn->nvertices; ++i) {
	  if (nn->vertices[i] == nn->test_vertice) {
	    w = nn->weights[i];
	    break;
	  }
	}
...
nnhpi* nnhpi_create(delaunay* d, int size)
{
    ...
    for (i = 0; i < d->npoints; ++i)
        ht_add(nn->ht_data, &d->points[i], &d->points[i]);
...
void nnhpi_interpolate(nnhpi* nnhpi, point* p)
{
    ...
    delaunay* d = nnpi->d;
    hash_table_t* ht_weights = nnhpi->ht_weights;
    nn_weights* weights;
    int i, found = 0;
    double tmn, tmx;
	...
	found = 1;
        ...
        ht_add(ht_weights, p, weights);

        if (nn_verbose) {
            if (nnpi->test_vertice == -1) {
                if (nnpi->n == 0)
                    fprintf(stderr, "weights:\n");
                ...
                for (i = 0; i < nnpi->nvertices; ++i) {
                    if (nnpi->vertices[i] == nnpi->test_vertice) {
                        w = nnpi->weights[i];
    ...
    for (i = 0; i < weights->nvertices; ++i) {
      if (weights->weights[i] < nnpi->wmin) {
            p->z = NaN;
	    p->z = d->points[weights->vertices[0]].z;
            return;
        }
	tmx = dmax(tmx, d->points[weights->vertices[i]].z);
	tmn = dmin(tmn, d->points[weights->vertices[i]].z);
        p->z += d->points[weights->vertices[i]].z * weights->weights[i];
    }
    p->z = dmin(tmx, dmax(tmn, p->z));

    if (found) ht_delete(ht_weights, p);
...
int main(int argc, char* argv[])
{
    ...
    while (i < argc) {
        ...
        case 'n':
            i++;
            if (i >= argc)
	      quit("no number of data points found after -n\n");
            nin = atoi(argv[i]);
            i++;
            if (i >= argc)
	      quit("no number of ouput points per side found after -i\n");
            nx = atoi(argv[i]);
            i++;
            break;
        case 'v':
    ...
    fflush(stdout);
    {
        hash_table_t* ht = nn->ht_data;

        printf("    input points: %d entries, %d table elements, %d filled elements\n", ht_getnentries(ht), ht_getnelems(ht), ht_getnoccupied(ht));
        ht = nn->ht_weights;
