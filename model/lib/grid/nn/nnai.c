 ...
 *
 *  File: model/lib/grid/nn/nnai.c
 *  
 *  Natural Neighbours Array Interpolator
 *
 *  Description:  `nnai' is a structure for conducting
 ...
 *
 *  $Id: nnai.c 5839 2018-06-28 03:38:07Z riz008 $
 *
 */
...
int main(int argc, char* argv[])
{
    ...
    while (i < argc) {
        ...
        case 'n':
            i++;
            if (i >= argc)
	      quit("no number of data points found after -i\n");
            nin = atoi(argv[i]);
            i++;
            if (i >= argc)
    ...
    fflush(stdout);
    nn = nnai_build(d, nout, xout, yout, nn_rule);
