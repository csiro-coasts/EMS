 ...
 * 
 *  Note that dentirification and anammox remove N from the system. Oxygen liberated 
 *  during anammox becomes water molecules, and is also removed from the system. 
 *
 *  Note that anammox involves nitrite, but since the model does not have nitrite we used nitrate. 
 *                 
 *  Copyright:
...
void nitrification_denitrification_anammox_init(eprocess* p)
{
  ...
  if (p->type == PT_SED){
    ...
    ws->r_nit_t0 = try_parameter_value(e, "r_nit_sed");
    if (isnan(ws->r_nit_t0)){
      ws->r_nit_t0 = 20.0/86400.0;
      eco_write_setup(e,"Code default of r_nit_sed = %e \n",ws->r_nit_t0);
    }
    
    ws->r_den_t0 = try_parameter_value(e, "r_den_sed");
    if (isnan(ws->r_den_t0)){
      ws->r_den_t0 = 0.1/86400.0;
      eco_write_setup(e,"Code default of r_den_sed = %e \n",ws->r_den_t0);
    }
    
    ws->KO_nit = try_parameter_value(e, "KO_nit_sed");
    if (isnan(ws->KO_nit)){
      ws->KO_nit= 500.0;
      eco_write_setup(e,"Code default of KO_nit_sed = %e \n",ws->KO_nit);
    }
    
    ws->KO_den = try_parameter_value(e, "KO_den_sed");
    if (isnan(ws->KO_den)){
      ws->KO_den = 10000.0;
      eco_write_setup(e,"Code default of KO_den_sed = %e \n",ws->KO_den);
    }
    
    ws->KO_ana = try_parameter_value(e,"KO_ana_sed");
    if (isnan(ws->KO_ana)){
      ws->KO_ana = 10000.0;
      eco_write_setup(e,"Code default of KO_ana_sed = %e \n",ws->KO_ana);
    }
    ws->r_ana_t0 = try_parameter_value(e,"r_ana_sed");
    if (isnan(ws->r_ana_t0)){
      ws->r_ana_t0 = 0.1/86400.0;
     eco_write_setup(e,"Code default of r_ana_sed = %e \n", ws->r_ana_t0);
    }
  }
  ...
  if (p->type == PT_WC){
    ...
    ws->r_nit_t0 = try_parameter_value(e, "r_nit_wc");
    if (isnan(ws->r_nit_t0)){
      ws->r_nit_t0 = 0.1/86400.0;
      eco_write_setup(e,"Code default of r_nit_wc = %e \n",ws->r_nit_t0);
    }
    
    ws->r_den_t0 = try_parameter_value(e, "r_den_wc");
    if (isnan(ws->r_den_t0)){
      ws->r_den_t0 = 0.0;
      eco_write_setup(e,"Code default of r_den_wc = %e \n",ws->r_den_t0);
    }
    
    ws->KO_nit = try_parameter_value(e, "KO_nit_wc");
    if (isnan(ws->KO_nit)){
      ws->KO_nit= 500.0;
      eco_write_setup(e,"Code default of KO_nit_wc = %e \n",ws->KO_nit);
    }
    
    ws->KO_den = try_parameter_value(e, "KO_den_wc");
    if (isnan(ws->KO_den)){
      ws->KO_den = 10000.0;
      eco_write_setup(e,"Code default of KO_den_wc = %e \n",ws->KO_den);
    }
   
    ws->KO_ana = try_parameter_value(e,"KO_ana_wc");
    if (isnan(ws->KO_ana)){
      ws->KO_ana = 10000.0;
      eco_write_setup(e,"Code default of KO_ana_wc = %e \n",ws->KO_ana);
    }
    ws->r_ana_t0 = try_parameter_value(e,"r_ana_wc");
    if (isnan(ws->r_ana_t0)){
      ws->r_ana_t0 = 0.1/86400.0;
     eco_write_setup(e,"Code default of r_ana_wc = %e \n", ws->r_ana_t0);
    }
  ...
  ws->Amm_fl_i = e->find_index(tracers, "Amm_fl", e);
...
void nitrification_denitrification_anammox_calc(eprocess* p, void* pp)
{
  ...
  double Nitrification = cv[ws->r_nit_i] * NH4 * Oxygen * Oxygen / (ws->KO_nit * ws->KO_nit + Oxygen * Oxygen);
  
  double Denitrification =  cv[ws->r_den_i] * NO3 * ws->KO_den / (ws->KO_den + Oxygen);

  double Anammox = 0.0;
  if (ws-> Amm_fl_i > -1){
    Anammox = cv[ws->r_ana_i] * min(NH4,NO3) * exp(- Oxygen / ws->KO_ana);
  }
  ...
  // Oxygen molecules released in anammox form water molecules - thus it is a loss of oxygen that needs to be accounted for in the mass balance.
  
  y1[ws->Oxygen_i] -= (Nitrification - Denitrification) * 48.0/14.01 ;
  
  if (ws-> Oxy_pr_i > -1)
    y1[ws->Oxy_pr_i] -= (Nitrification - Denitrification) * 48.0/14.01 * dz * SEC_PER_DAY;
}
