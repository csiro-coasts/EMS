...
void zooplankton_large_carnivore_spectral_grow_wc_init(eprocess* p)
{
  ecology* e = p->ecology;
  ...
  ws->phi_ZS_i = find_index_or_add(e->cv_cell, "phi_ZS", e);
  //if (ws->with_mpb)
...
void zooplankton_large_carnivore_spectral_grow_wc_postinit(eprocess* p)
{
  ...
  emstag(LINFO,"eco:zooplankton_large_carnivore_spectral_grow_wc:postinit","%sCalculating consumption of MPB",(ws->with_mpb?"":"NOT "));
...
void zooplankton_large_carnivore_spectral_grow_wc_precalc(eprocess* p, void* pp)
{
  workspace* ws = p->workspace;
  cell* c = (cell*) pp;
  double* y = c->y;
  ...
  cv[ws->phi_ZS_i] = phi(ws->meth, ws->ZSrad, 0.004 * pow(ws->ZSrad, 0.26), 0.0, ws->rad, swim, 0.0, ws->TKEeps, vis, 1000.0, temp);
...
void zooplankton_large_carnivore_spectral_grow_wc_calc(eprocess* p, void* pp)
{
  workspace* ws = p->workspace;
  ...
  */
  ...
  double grZS = graze * phi_ZS / max_enc ;
  ...
  Oxy_pr = -DIC_release * red_W_O / red_W_C * sigmoid;
 ...
 if (ws->ZooL_N_rm_i  > -1)
   ...
   y1[ws->ZooL_N_rm_i] += (grZS * ZooS_N + grL * PhyL_N + grD * PhyD_N + grTricho * Tricho_N + grMPB * MPB_N) * red_W_C * SEC_PER_DAY;
