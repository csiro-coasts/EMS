 ...
 *  
 *  File: model/lib/ecology/process_library/zooplankton_small_grow_wc.c
 *  
 *  Description:
...
#include "zooplankton_small_grow_wc.h"
...
void zooplankton_small_grow_wc_init(eprocess* p)
{
    ...
    ws->phi_PS_ZS_i = find_index_or_add(e->cv_cell, "phi_PS_ZS", e);
...
void zooplankton_small_grow_wc_precalc(eprocess* p, void* pp)
{
    ...
    cv[ws->phi_PS_ZS_i] = phi(ws->meth, ws->PSrad, 0.004 * pow(ws->PSrad, 0.26), 0.0, ws->rad, swim, 0.0, ws->TKEeps, vis, 1000.0, temp);
...
void zooplankton_small_grow_wc_calc(eprocess* p, void* pp)
{
    ...
    double phi_PS_ZS = cv[ws->phi_PS_ZS_i];
    double max_enc = PhyS_N * phi_PS_ZS;
    ...
    y1[ws->PhyS_N_i] -= (graze != 0.0) ? graze * PhyS_N * phi_PS_ZS / max_enc : 0.0;
