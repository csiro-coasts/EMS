 ...
 *  
 *  File: model/hd/forcings/storm.c
 *  
 *  Description:
 ...
 *  
 *  $Id: storm.c 5841 2018-06-28 06:51:55Z riz008 $
 *
 */
                   ...
                   /* dr = 1 : low pressure system */
{
  ...
  for (cc = 1; cc <= geom->b2_t; cc++) {
    ...
    if (d <= dmax) {
      ...
      /* Coriolis */
      f = 0.5 * (master->u1c5[c] + master->u1c5[geom->xp1[c]]);
      f = fabs(f);
  ...
  for (cc = 1; cc <= geom->b2_e1; cc++) {
    c = geom->w2_e1[cc];
    xp1 = geom->xp1[c];
    master->wind1[xp1] = 0.5 * (master->wind1[c] + master->wind1[xp1]);
  }
  for (cc = 1; cc <= geom->b2_e2; cc++) {
    c = geom->w2_e2[cc];
    yp1 = geom->yp1[c];
    master->wind2[yp1] = 0.5 * (master->wind2[c] + master->wind2[yp1]);
  }
}
