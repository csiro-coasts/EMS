...
double webf_event(sched_event_t *event, double t)
{
  ...
  if (t >= (event->next_event - SEPS)) {
    ...
    if (strlen(data->interp_type)) {
      /* Use the grid library as per the interpolation type specified */
      if (ub_id >= 0)
    ...
    /* Read the wave-induced forcing data, and if necessary rotate onto the grid */
    if (fx_id >= 0 && fy_id >= 0)
      if (!data->wcs) {
        for (ii = 1; ii < geom->b2_e1; ++ii) {
          i = geom->w2_e1[ii];
          ...
          wavey = hd_ts_multifile_eval_xy(data->ntsfiles, data->ts, data->fy_id,
                                          t, geom->u1x[i], geom->u1y[i]);
          master->wave_Fx[i] = wavex * geom->costhu1[i] + wavey * geom->sinthu1[i];
	}
        for (ii = 1; ii < geom->b2_e2; ++ii) {
          i = geom->w2_e2[ii];
          wavex = hd_ts_multifile_eval_xy(data->ntsfiles, data->ts, data->fx_id,
                                          t, geom->u2x[i], geom->u2y[i]);
          wavey = hd_ts_multifile_eval_xy(data->ntsfiles, data->ts, data->fy_id,
                                          t, geom->u2x[i], geom->u2y[i]);
          master->wave_Fy[i] = -wavex * geom->sinthu2[i] + wavey * geom->costhu2[i];
	}
      } else {
        for (ii = 1; ii < geom->b2_e1; ++ii) {
          i = geom->w2_e1[ii];
        ...
        for (ii = 1; ii < geom->b2_e2; ++ii) {
          ...
          wavey = hd_ts_multifile_eval_xy(data->ntsfiles, data->ts, data->fy_id,
                                          t, geom->u2x[i], geom->u2y[i]);
          master->wave_Fy[i] = wavey;
	}
      }
    /* Read the stokes velocity forcing data, and if necessary rotate onto the grid */
      ...
      if (!data->stcs) {
        for (ii = 1; ii < geom->b2_t; ++ii) {
          ...
          wavey = hd_ts_multifile_eval_xy(data->ntsfiles, data->ts, data->ste2_id,
                                          t, geom->cellx[i], geom->celly[i]);
          master->wave_ste1[i] = wavex * geom->costhu1[i] + wavey * geom->sinthu1[i];
	}
        for (ii = 1; ii < geom->b2_t; ++ii) {
          i = geom->w2_t[ii];
          wavex = hd_ts_multifile_eval_xy(data->ntsfiles, data->ts, data->ste1_id,
          ...
          wavey = hd_ts_multifile_eval_xy(data->ntsfiles, data->ts, data->ste2_id,
                                          t, geom->cellx[i], geom->celly[i]);
          master->wave_ste2[i] = -wavex * geom->sinthu2[i] + wavey * geom->costhu2[i];
	}
      } else {
        for (ii = 1; ii < geom->b2_t; ++ii) {
          i = geom->w2_t[ii];
    ...
    /* Read the wave supported wind friction velocities and convert to a stress, */
    /* rotating onto the grid. */
	...
	wavey = hd_ts_multifile_eval_xy(data->ntsfiles, data->ts, data->aw2_id,
					t, geom->cellx[i], geom->celly[i]);
	master->tau_w1[i] = RHO_0 * (wavex * geom->costhu1[i] + wavey * geom->sinthu1[i]);
	master->tau_w2[i] = RHO_0 * (-wavex * geom->sinthu2[i] + wavey * geom->costhu2[i]);
      }
    }

    /* Read the wave to ocean friction velocities and convert to a stress, */
    /* rotating onto the grid. */
	...
	wavey = hd_ts_multifile_eval_xy(data->ntsfiles, data->ts, data->wo2_id,
					t, geom->cellx[i], geom->celly[i]);
	master->tau_diss1[i] = -RHO_0 * (wavex * geom->costhu1[i] + wavey * geom->sinthu1[i]);
	master->tau_diss2[i] = -RHO_0 * (-wavex * geom->sinthu2[i] + wavey * geom->costhu2[i]);
      }
    }
