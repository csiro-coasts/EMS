...
double wind_event(sched_event_t *event, double t)
{
  ...
  if (t >= (event->next_event - SEPS)) {
	...
	/* Wind e2 component at u2 points                            */
	for (ii = 1; ii <= geom->b2_e2; ++ii) {
	  ...
	  windy = data->wind_scale
	    * hd_ts_multifile_eval_xy(data->ntsfiles, data->tsfiles,
				      ...
				      t, geom->u2x[i], geom->u2y[i]);
	  cd = data->windstress(master, data, &windx, &windy, i);
	  master->wind2[i] = -windx * geom->sinthu2[i]
	    + windy * geom->costhu2[i];
	...
	memcpy(master->swind2, master->wind2, geom->sgsizS * sizeof(double));
      }
    } else {
      /*-------------------------------------------------------------*/
	...
	/* Wind stress e2 component at u2 points                     */
	for (ii = 1; ii <= geom->b2_e2; ++ii) {
	  i = geom->w2_e2[ii];
	  windx = data->wind_scale
	    * hd_ts_multifile_eval_xy(data->ntsfiles, data->tsfiles,
    ...
    if (master->waves & STOKES_WIND) {
      if (master->tau_w1 && master->tau_diss1) {
	for (ii = 1; ii <= geom->b2_e1; ++ii) {
	  i = geom->w2_e1[ii];
	  master->wind1[i] -= (master->tau_w1[i] + master->tau_diss1[i]);
	}
      }
      if (master->tau_w2 && master->tau_diss2) {
	for (ii = 1; ii <= geom->b2_e2; ++ii) {
	  i = geom->w2_e2[ii];
	  master->wind2[i] -= (master->tau_w2[i] + master->tau_diss2[i]);
	}
      }
    }
  }
