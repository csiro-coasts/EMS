 ...
 *  
 *  File: model/hd/forcings/forcings.c
 *  
 *  Description:
                      ...
                      double *p, double conv)
{
  ...
  for (i = 1; i <= geom->sgnumS; ++i)
    p[i] = conv * ts_eval_xyz(ts, id, t, geom->cellx[i], geom->celly[i], 0.0);
}
			   ...
			   int ntsfiles, double *p, double conv)
{
  ...
  /* Evaluate values at cell centres */
  for (i = 1; i <= geom->sgnumS; ++i)
    p[i] = conv * hd_ts_multifile_eval_xy(ntsfiles, ts, id,
