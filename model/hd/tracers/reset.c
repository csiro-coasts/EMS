 ...
 *  
 *  $Id: reset.c 5841 2018-06-28 06:51:55Z riz008 $
 *
 */
...
static int get_datum(int ntsfiles, timeseries_t **tsfiles,
  ...
  if ((t + reset->dt / 10.0) >= (reset->tnext - tsync)) {
    if (master->tmode & SP_ORIGIN) {
      ...
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      ...
			      tec, ts3, (mode|U1GEN));
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "u2", master->u2, tsin,
			      tec, ts3, (mode|U2GEN));
    }
    if (master->tmode & GLOBAL) {
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "u", tp->tpd->u1, tsin, tec, ts3, (mode|U1GEN));
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "v", tp->tpd->u2, tsin, tec, ts3, (mode|U2GEN));
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "eta", tp->tpd->eta, tsin,
			      tec, ts2, (mode|VEL2D));
    }
    if (master->tmode & XYZ_TINT) {
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "u", tp->tpd->u1, tsin, tec, ts3, (mode|U1GEN));
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "v", tp->tpd->u2, tsin, tec, ts3, (mode|U2GEN));
    ...
    if (master->tmode & SP_FFSL) {
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "u1vmean", tp->tpd->u1vm, tsin,
			      tec, ts3, (mode|U1GEN));
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "u2vmean", tp->tpd->u2vm, tsin,
			      tec, ts3, (mode|U2GEN));
    }
    /* Read in mandatory variables                                   */
    if (!(master->tmode & (SP_ORIGIN|GLOBAL|XYZ_TINT))) {
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "u1", tp->tpd->u1, tsin,
			      tec, ts3, (mode|U1GEN));
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "u2", tp->tpd->u2, tsin,
			      tec, ts3, (mode|U2GEN));
      if (!(master->conserve & CONS_W) || master->conserve & (CONS_PSS|CONS_WS)) {
	hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
				reset->tsnames, "w", tp->tpd->w, tsin,
    ...
    if (!(master->tmode & GLOBAL)) {
      if (!master->do_closure)
	hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
				reset->tsnames, "Kz", tp->tpd->Kz, tsin,
				tec, ts3, mode);
      
      if (!(master->conserve & NOINIT)) {
	memcpy(tp->tpc->oldeta, tp->tpc->d7, tp->tpg->sgsizS * sizeof(double));
	hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
				reset->tsnames, "eta", tp->tpd->eta, tsin,
				tec, ts2, (mode|VEL2D));
	memcpy(tp->tpc->d7, tp->tpd->eta, tp->tpg->sgsizS * sizeof(double));
      }
    }
    ...
    if (master->tmode & DO_SWR) {
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "swr", tp->tpd->light, tsin,
			      tec, ts2, (mode|VEL2D));
    }

    /* Read in fluxes if not advected with LAGRANGE                  */
    /*
    if (!(master->trasc & LAGRANGE)) {
      hd_ts_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			   reset->tsnames, "u1flux3d", master->u1flux3d, t,
			   vec, ns3, mode);
      hd_ts_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			   reset->tsnames, "u2flux3d", master->u2flux3d, t,
			   vec, ns3, mode);
    }
    */
	...
	!master->trinfo_3d[master->tno].diffuse)
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "temp", tp->tpd->temp, tsin,
			      tec, ts3, mode);
    if (!master->trinfo_3d[master->sno].advect && 
	!master->trinfo_3d[master->sno].diffuse)
      hd_trans_multifile_eval(master, reset->ntsfiles, reset->tsfiles,
			      reset->tsnames, "salt", tp->tpd->sal, tsin,
			      tec, ts3, mode);
