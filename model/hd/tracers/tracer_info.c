 ...
 *  
 *  File: model/hd/tracers/tracer_info.c
 *  
 *  Description:
...
int tracer_write(parameters_t *params, FILE *op, win_priv_t *wincon)
{
  ...
  if (params->roammode & (A_RECOM_R1|A_RECOM_R2) || params->runmode & PRE_MARVL) {
    ...
    for (tn = 0; tn < master->ntrS; tn++)
      num_tr += ecology_count_tracerstats(&master->trinfo_2d[tn]);
    /* Count any ecology 3D tracerstats */
    for (tn = 0; tn < master->ntr; tn++)
      num_tr += ecology_count_tracerstats(&master->trinfo_3d[tn]);
#endif
    fprintf(op, "\n# Tracers\n");
...
#if defined(HAVE_ECOLOGY_MODULE)
    /* For RECOM, dump out any ecology tracer stats */
    if (!(params->runmode & PRE_MARVL)) {
      /* 2D */
      for (tn = 0; tn < master->ntrS; tn++) {
	tracer_info_t *tracer = &master->trinfo_2d[tn];
	n = ecology_write_tracerstat(master, op, tracer, n);
      }
      /* 3D */
      for (tn = 0; tn < master->ntr; tn++) {
