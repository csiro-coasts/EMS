 ...
 *  
 *  $Id: sigma.c 5841 2018-06-28 06:51:55Z riz008 $
 *
 */
  ...
  )
{
  ...
  if (wincon->sigma) {
    ...
    for (n = 0; n < window->nobc; n++) {
      if (open[n]->ocodex & R_EDGE) {
        for (cc = 1; cc <= open[n]->no2_e1; cc++) {
          c = open[n]->obc_e1[cc];
          wincon->Ds[c] = wincon->Ds[open[n]->obc_t[cc]];
        }
      }
    ...
    memcpy(windat->topz, windat->eta, window->sgsizS * sizeof(double));
}
  ...
  )
{
  ...
  if (bmax == bmin) {
    ...
    /* Increased near-surface resolution for ROAM 14.07.2017 */
  ...
  )
{
  ...
  fi = kk[1][3] * a[cfl] + kk[2][3] * a[cf] + kk[3][3] * a[cfr] +
    kk[1][2] * a[cl] + kk[2][2] * a[c] + kk[3][2] * a[cr] +
    kk[1][1] * a[cbl] + kk[2][1] * a[cb] + kk[3][1] * a[cbr];
  return (fi);
}
...
/*-------------------------------------------------------------------*/
/* Routine to apply a convolution smoothing filter to an element of  */
/* a 2-D array within a window.                                      */
/*-------------------------------------------------------------------*/
double cvol1w(geometry_t *window,  /* Window geometry                */
	      double *a,           /* Array to smooth                */
