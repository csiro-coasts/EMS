 ...
 *  
 *  File: model/hd/inputs/sourcesink.c
 *  
 *  Description:
...
/* flux parameters.                                                  */
/*-------------------------------------------------------------------*/
void sourcesink_init(parameters_t *params,  /* Input parameters structure */
...
{
  geometry_t *geom = master->geom;  /* Sparse global geometry structure */
  int n, s, m = 0;
  int t;
  ...
  hd_pss_read(params->prmname, master->timeunit, &master->pss,
           &master->npss, master, hd_xyztoindex_m, hd_get_tracer_index_m, hd_pss_vec_m);
  */
  hd_pss_read(params->prmname, master->timeunit, &master->pss,
           &master->npss, master, hd_xyztoindex_m, hd_get_tracer_index_m, hd_pss_vec_m,
	   hd_pss_region_m);
  ...
  for (s = 0; s < master->npss; s++) {
    ...
    if (geom->s2i[pss->e1[0]] == NOTVALID && geom->s2j[pss->e1[0]] ==NOTVALID &&
	geom->s2k[pss->e1[0]] ==NOTVALID)
      hd_quit
...
/*-------------------------------------------------------------------*/
void sourcesink(master_t *master  /* Master data structure */
  )
{
  int s, n, nn;
  ...
  memset(master->waterss2d, 0, geom->sgsizS);
  memset(master->waterss, 0, geom->sgsiz);
...
/* sinks in the window.                                              */
/*-------------------------------------------------------------------*/
void ss_water(geometry_t *window, /* Processing window */
              window_t *windat, /* Window data structure */
              win_priv_t *wincon  /* Window geometry / constants */
  )
{
  ...
  /* Clear the 2-d and 3-d water source/sink arrays */
  memset(windat->waterss, 0, window->sgsiz * sizeof(double));
  memset(windat->waterss2d, 0, window->sgsizS * sizeof(double));
  ...
  if (master->trasc == FFSL && master->conserve & CONS_PSS && !(master->conserve & CONS_NOF)) {
    int n, nn, cs;
	...
	for (nc = 0; nc < p->vc; nc++) {
	  ...
	  if(isnan(windat->waterss2d[cs])) hd_quit("ss_water: NaN in transport file at source %s : %d\n", p->name, cs);
	}
...
/* of each cell;                                                     */
/*-------------------------------------------------------------------*/
void ss_tracer(geometry_t *window,  /* Processing window */
               window_t *windat,  /* Window data structure */
               win_priv_t *wincon,  /* Window geometry / constants */
               int n, double *dtracer, double dt)
{
  ...
  for (s = 0; s < wincon->npss; s++) {
    ...
    /* Loop over horizontal locations                              */
    for (nc = 0; nc < p->vc; nc++) {
      /* Find position of source/sink horizontally and vertically, and */
      /* truncate to available water column. It would be nice if this */
      /* didn't have to be done repeatedly for each tracer (each time */
	...
	if (pdz > 0.0) {
	  ...
	  while (c != window->zm1[c]) {
	    ...
	    /* If this source/sink is not associated with temp or sal  */
	    /* and a volume influx is specified (trf = CONC_NOTR) then */
	    /* assume tracer is input with the ambient water column    */
	    /* concentration. If this is not done, then these tracer   */
	    /* concentrations will change due to violations of volume  */
	    /* conservation, which subsequently impacts density and    */
	    /* flow.                                                   */

	    /* For the FFSL transport mode with volume inputs, use the */
	    /* volume fluxes in each cell saved to wmean and copied to */
	    /* waterss. Note that frac uses ctop based on topz, which  */
	    /* is the oldeta value and is out of sync in transport     */
	    /* mode. Using the saved waterss directly ensures exact    */
	    /* volume flux input in transport mode.                    */
	    if (p->watertsid >= 0) {
...
void hd_pss_read(char *name,       /* File name */
              char *t_units, pss_t **pss, int *np, void *model_data,  /* Data
                                                                         for
                                                                         xyzijk
                                                                         and
                                                                         trI
...
int pss_vec_m(master_t *master, pss_t *pss)
{
  ...
  if (pss->vc) {
    ...
    for(cc = 1; cc <= n; cc++) {
      i = min(pss->iloc[cc], geom->nce1 - 1);
      j = min(pss->jloc[cc], geom->nce2 - 1);
      c = geom->map[geom->nz-1][j][i];
      if (c > 0 && c <= geom->ewetS) pss->vc++;
    }
...
int pss_vec_w(geometry_t *window, pss_t *pss)
{
  ...
  if (n != 0) {
	...
	i = window->s2i[c];
	j = window->s2j[c];
	if (i == pss->iloc[nn+1] && j == pss->jloc[nn+1]) {
	  mask[pss->vc] = c; 
	  pss->vc++;	  
	  break;
	}
