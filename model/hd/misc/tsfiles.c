                          ...
                          double tstop)
{
  ...
  if (ntsfiles > 0) {
    ...
    hd_quit("hd_ts_multifile_check: No timeseries files specified for %s.\n", var);
    return 0;
  }
			  ...
			  int *vec, int nvec, int mode)
{
  ...
  if (mode & SP_EXACT) {
    hd_ts_multifile_eval_sparse(ntsfiles, tsfiles, names, var,
				dumpdata->w1, t, nvec, master->thIO);
    unpack_sparse(vec, nvec, dumpdata->w1, v);
  }
			     ...
			     int *vec, int nvec, int mode)
{
  ...
  if (mode & SP_EXACT) {
    hd_ts_multifile_eval_sparse(ntsfiles, tsfiles, names, var,
				tp->w1, t, nvec, master->thIO);
    unpack_sparse(vec, nvec, tp->w1, v);
  }
  else if (mode & SP_TINT) {
    hd_ts_multifile_eval_isparse(ntsfiles, tsfiles, names, var,
				 tp->w1, t, nvec);
...
int dump_choose_multifile(datafile_t *df, double t)
{
  ...
  if (fd != NULL) {
	...
	if (r0 == r1 && fabs(t - df->records[r0]) > START_EPS) {
	  ...
	  nc_close(df->ncid);
	  df->ncid = -1;
	} else if (r0 < r1 && fabs(t - df->records[r1]) < START_EPS) {
	  /* Close enough to the end (RAS 19/4/2017) */
	  /* Add this in and invert logic below to handle the case */
	  /* where due to some precision issue we get a t which is a */
	  /* microsecond before the time in the file */
	  return(r1);
			  ...
			  cstring * names)
{
  ...
  ns2 = geom->ns2; ns3 = geom->ns3;
  nce1 = geom->nce1; nce2 = geom->nce2;
  nz = geom->nz;

  if (ntsfiles > 0) {
    for (i = 0; i < ntsfiles; ++i) {
      char *fname = fv_get_filename(names[i], buf);
      if ((n = read_sparse_dims(fname, ns2, ns3, nce1, nce2, nz))) {
	if (n == 2) return(1);
	if ((fp = fopen(fname, "r")) != NULL) {
	  ...
	  if (prm_skip_to_end_of_key(fp, "multi-netcdf-version")) {
		...
		if ((isnc = read_sparse_dims(fname, ns2, ns3, nce1, nce2, nz))) {
		  fclose(fp);
		  if (isnc == 1) hd_warn("File %d in %s not a sparse file.\n", i, fname);
		  if (isnc == 2) hd_warn("Sparse file %d in %s doesn't have correct dimensions.\n", i, fname);
		  return(1);
		}
	      }
	    }
		      ...
		      
{
  ...
  if (nc_open(name, NC_NOWRITE, &ncid) == NC_NOERR) {
    ...
    nc_close(ncid);
    if (ns2 != ms2 || ns3 != ms3 || nce1 != mce1 || nce2 != mce2 || nz != mz-1) {
      hd_warn("Input file: ns2=%d, ns3=%d nce1=%d, nce2=%d, nz=%d\n",ns2, ns3, nce1, nce2, nz);
      hd_warn("Trans file: ns2=%d, ns3=%d nce1=%d, nce2=%d, nz=%d\n",ms2, ms3, mce1, mce2, mz-1);
      return(2);
    }
    return(0);
  }
...
/*-------------------------------------------------------------------*/
/* Routine to perform a global fill and smooth the AVHRR SST data.   */
/*-------------------------------------------------------------------*/
void process_avhrr(master_t *master, /* Master data                  */
		   timeseries_t *ts) /* Timeseries file              */
{
  geometry_t *geom = master->geom;
  int do_global_fill = 1;            /* Perform global filling       */
  int do_smooth = 1;                 /* Perform smoothing            */
  int do_median = 1;                 /* Perform median filtering     */
  double min_sst = 5.0;              /* Minimum SST threshold        */
  double sfact = 1.0;                /* Scaing for std_dev threshold */
  double sr = 1100.0;                /* AVHRR resolution             */
  double tol = 1e-5;                 /* Tolerance for land mask      */
  int c, c1, cc, n, sn;
  int edge, edge_pos, edge_len;
  int finished;
  double *sst = master->avhrr;
  double cv = ts_get_att_value(ts, "sst", "cloud_flag");
  double lv = ts_get_att_value(ts, "sst", "land_flag");
  double* aa, fval;
  ...
  for(cc = 1; cc <= geom->b2_t; cc++) {
    c = geom->w2_t[cc];    
    if (!(is_fin(sst[c], tol, lv, min_sst))) {
      mean_sst += sst[c];
      std_dev += sst[c] * sst[c];
      n += 1;
    }
  ...
  set_lateral_bc_eta(master->avhrr, geom->nbptS, geom->bpt, geom->bin,
		     geom->bin2, 0);
  for(cc = 1; cc <= geom->n2_t; cc++) {
    ...
    aa[c] = sst[c];
    finished = is_fin(sst[c], tol, lv, min_sst);
    /* Find the nearest valid value */
    for (n = 0; n < geom->b2_t && finished; n++) {
      aa[c] = sst[c1];
      finished = is_fin(aa[c], tol, lv, min_sst);
    ...
    if (finished) {
      ...
      for (c2 = 0; c2 < ns; c2++) {
	c1 = st[c2];
	if (!(is_fin(aa[c1], tol, lv, min_sst))) {
	  ms += aa[c1];
	  n += 1;
	}
      }
  ...
  /*-----------------------------------------------------------------*/
  /* Smooth the sst data, excluding cloud locations                  */
  set_lateral_bc_eta(master->avhrr, geom->nbptS, geom->bpt, geom->bin,
		     geom->bin2, 0);
  for (cc = 1; cc <= geom->b2_t; cc++) {
    c = geom->w2_t[cc];
    aa[c] = sst[c];
    c1 = geom->xm1[c];
    edge = L_EDGE; edge_pos = 1; edge_len = 2;
    fval = 1.0;
    while (edge_len == 2) {
      if (fabs(sst[c1] - cv) > tol) {
	aa[c] += sst[c1];
	fval += 1.0;
      }
      c1 = c_search(geom, c1, &edge, &edge_len, &edge_pos);
...
/*-------------------------------------------------------------------*/
/* Returns the sparse coordinate of a widening, anticlockwise search */
/* around a given cell, starting in the top left (NW) corner. The    */
/* variables edge, edge_len and edge_pos must be initialised to      */
/* L_EDGE, 2 and 0 respectively.                                     */
/*-------------------------------------------------------------------*/
int c_search(geometry_t *geom, int c, int *edge, int *edge_len, int *edge_pos)
{
  int *map = geom->ym1;
...
int is_fin(double sst, double tol, double mask, double lim)
{
  ...
  if (fabs(sst - mask) < tol)     /* sst is equal to the land mask */
    return(1);
  else if(sst < lim)              /* sst is less than the lower limit */
    return(1);
