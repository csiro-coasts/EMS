 ...
 *  
 *  File: model/hd/outputs/dumpfile.c
 *  
 *  Description:
...
#define STD_ALL_VARS "u1av u2av wtop topz eta wind1 wind2 patm u1 u2 w dens dens_0 Kz Vz u1bot u2bot Cd flag u1vh u2vh "
...
static void *df_std_create(dump_data_t *dumpdata, dump_file_t *df);
static void df_std_write(dump_data_t *dumpdata, dump_file_t *df, double t);
static void df_std_close(dump_data_t *dumpdata, dump_file_t *df);
...
/* Point array output file prototypes */
...
double df_filter(dump_file_t *df, double **a, unsigned long **flag, 
...
static struct {
...
} df_func_map[] = {
  {
    ...
    "restart", df_restart_create, df_restart_write, df_restart_close, df_std_reset, 1}, {
...
double dump_event(sched_event_t *event, double t)
{
  ...
  TIMING_DUMP(1," master_fill");

  /* Interpolate on the master for zoomed grids                    */
  if (master->dozoom != NOZOOM) {
    global_interp(geom, master->eta, geom->nzinS);
    global_interp_e1(geom, master->u1av, geom->nzine1S);
    global_interp_e2(geom, master->u2av, geom->nzine2S);
    ...
    for (i = 0; i < master->ntr; i++)
      global_interp(geom, master->tr_wc[i], geom->nzin);
  }
  ...
  for (f = 0; f < dumpdata->ndf; f++) {
	...
	for (n = 1; n <= master->nwindows; n++) {
	  for (bn = 0; bn < window[n]->nobc; bn++) {
		...
		c = open->obc_e2[cc];
		gc = window[n]->wsa[c];
		master->u2av[gc] = windat[n]->u2av[c];
	      }
	...
	s2c_2d(geom, master->u2av, dumpdata->u2av, geom->nce1, geom->nfe2);
	s2c_3d(geom, master->u1, dumpdata->u1, geom->nfe1, geom->nce2, geom->nz);
	s2c_3d(geom, master->u2, dumpdata->u2, geom->nce1, geom->nfe2, geom->nz);
	dov2d = 0;
      }
      /* Dump */
      if (!(df->finished)) {
...
void dumpfile_close(dump_data_t *dumpdata, double t, FILE * fp)
{
  ...
  if (dumpdata->master->mpi_rank)
    return;
  
  /* Loop through dump file dumplist, writing any file which needs it */
  for (f = 0; f < dumpdata->ndf; f++) {
    /* Write to file if its still open */
    if (!(dumpdata->dumplist[f].finished)) {
      dumpdata->dumplist[f].landfill(dumpdata);
      dump_bathy_mask(dumpdata, dumpdata->dumplist[f].bathymask);
      dumpdata->dumplist[f].write(dumpdata, &dumpdata->dumplist[f], t);
...
void read_filter(dump_file_t *df, FILE *fp, char *key)
{
  ...
  if (prm_read_char(fp, key, buf)) {
    ...
    f->k = d_alloc_1d(f->size);
    for (n = 0; n < f->size; n++)
      f->k[n] = 1.0 / (double)f->size;
    f->map = i_alloc_1d(f->size);
...
static void *df_std_create(dump_data_t *dumpdata, dump_file_t *df)
{
  ...
  /* Do NOT create the file if not master */
  ...
  /*
   * Add cascade search land fill map, if mapped and 
   *           only for restart files
   */
  if (df->create == df_restart_create)
    if (dumpdata->fmap_i != NULL && dumpdata->fmap_j != NULL) {
      dims[0] = kcentreid;
      dims[1] = jcentreid;
      dims[2] = icentreid;
      nc_def_var(cdfid, "fmap_i", NC_INT, 3, dims, &vid);
      nc_def_var(cdfid, "fmap_j", NC_INT, 3, dims, &vid);
    }
		  ...
		  int i, int j)
{
  ...
  for (k = 0; k < dumpdata->nz; k++) {
    dumpdata->w3[k][j][i] = df_filter(df, a[k], dumpdata->flag[k], 
		  ...
		  int is, int ie, int js, int je)
{
  ...
  for (j = js; j < je; j++)
    for (i = is; i < ie; i++)
      dumpdata->w2[j][i] = df_filter(df, a, dumpdata->flag[dumpdata->nz-1], 
				     dumpdata->nce1, dumpdata->nce2, i, j);
}
...
static void df_std_writegeom(dump_data_t *dumpdata, dump_file_t *df)
{
  ...
  {
    int fmap_i_vid = ncw_var_id(fid, "fmap_i");
    int fmap_j_vid = ncw_var_id(fid, "fmap_j");
    if (fmap_i_vid > -1 && fmap_j_vid > -1) {
      start[0] = df->klower;
      start[1] = df->jlower;
      start[2] = df->ilower;
      count[0] = df->nz;
      count[1] = df->nce2;
      count[2] = df->nce1;
      nc_i_writesub_3d(fid, fmap_i_vid, start, count, dumpdata->fmap_i);
      nc_i_writesub_3d(fid, fmap_j_vid, start, count, dumpdata->fmap_j);
    }
  }
  ncw_sync(df->name,fid);
		      ...
		      size_t * count, int ***values)
{
  ...
  offset = (nd > 3);
  nvals =
    (int ***)i_alloc_3d(count[offset + 2], count[offset + 1],
			count[offset]);
  for (k = 0; k < count[offset]; ++k)
    for (j = 0; j < count[offset + 1]; ++j)
      for (i = 0; i < count[offset + 2]; ++i)
  ...
  nc_put_vara_int(fid, varid, nstart, count, nvals[0][0]);
...
static double get_vector_component(dump_data_t *dumpdata, double vi,
                                   double vj, int i, int j, int mode);
...
static double get_var_value_2d(dump_file_t *df, df_parray_var_t *var,
                               double **values, int xylocation,
                               unsigned long **flag, double fi, double fj);
...
*/
  ...
  for (n = 0; n < df->nvars; n++) {
    ...
    if (var->ndims == 2) {
      if (var->vector_mode == VM_NONE)
        df_parray_writesub_2d(dumpdata, df, n, get_data_2d(var));
      else
        df_parray_writevec_2d(dumpdata, df, n,
                              get_data_vector_2d(var, 0),
                              get_data_vector_2d(var, 1));
      ...
      if (var->vector_mode == VM_NONE)
        df_parray_writesub_3d(dumpdata, df, n, get_data_3d(var),
                              klower, nz);
      else
        df_parray_writevec_3d(dumpdata, df, n,
                              get_data_vector_3d(var, 0),
                              get_data_vector_3d(var, 1), klower, nz);
    }
  }
  ...
  if(params->parray_inter_botz)
  {
    /*UR interpolate in the same fashion as other variables */
      ...
      if (grid_xytofij(master->xyij_tree, df->x[i], df->y[i], &fi, &fj) >= 0) {
        v[i] = get_var_value_2d(df, NULL, dumpdata->botz, CL_CENTRE,
                                dumpdata->flag[dumpdata->nz - 1], fi, fj);
      } else
        v[i] = 0.0;
    }
  }else {
    /*UR don't interpolate, just output the value of the columnthe location falls into*/
    for (i = 0; i < df->npoints; ++i) {
      if (grid_xytofij(master->xyij_tree, df->x[i], df->y[i], &fi, &fj) >= 0) {
                          ...
                          char *name, df_parray_var_t *var)
{
  ...
  if (strcmp(name, "eta") == 0) {
    set_data(var, (void **)&dumpdata->eta);
    strcpy(var->units, dumpdata->lenunit);
    strcpy(var->long_name, "Surface elevation");
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }

  else if (strcmp(name, "wtop") == 0) {
    set_data(var, (void **)&dumpdata->wtop);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Surface vertical current");
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }

  else if (strcmp(name, "topz") == 0) {
    set_data(var, (void **)&dumpdata->topz);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Surface z level");
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }

  else if (strcmp(name, "patm") == 0) {
    set_data(var, (void **)&dumpdata->patm);
    strcpy(var->units, "Pa");
    strcpy(var->long_name, "Atmospheric pressure");
    var->valid_range[0] = 91000.0;
    var->valid_range[1] = 111000.0;
  }

  else if (strcmp(name, "Cd") == 0) {
    set_data(var, (void **)&dumpdata->Cd);
    strcpy(var->units, "");
    strcpy(var->long_name, "Bottom drag coefficient");
    var->valid_range[0] = 0.0;
    var->valid_range[1] = 1.0;
  }

  else if (strcmp(name, "Kz") == 0) {
    set_data(var, (void **)&dumpdata->Kz);
    set_zloc(var, CL_CENTRE);
    var->ndims = 3;
    strcpy(var->units, "");
    strcpy(var->long_name, "Kz");
    var->valid_range[0] = 0.0;
    var->valid_range[1] = 0.1;
  }

  else if (strcmp(name, "Vz") == 0) {
    set_data(var, (void **)&dumpdata->Vz);
    set_zloc(var, CL_CENTRE);
    var->ndims = 3;
    strcpy(var->units, "");
    strcpy(var->long_name, "Vz");
    var->valid_range[0] = 0.0;
    var->valid_range[1] = 0.1;
  }

  else if (strcmp(name, "dens") == 0) {
    set_data(var, (void **)&dumpdata->dens);
    set_zloc(var, CL_CENTRE);
    var->ndims = 3;
    strcpy(var->units, "kg m-3");
    strcpy(var->long_name, "Density");
    var->valid_range[0] = 980.0;
    var->valid_range[1] = 1070.0;
  }

  else if (strcmp(name, "dens_0") == 0) {
    set_data(var, (void **)&dumpdata->dens_0);
    set_zloc(var, CL_CENTRE);
    var->ndims = 3;
    strcpy(var->units, "kg m-3");
    strcpy(var->long_name, "Potential density");
    var->valid_range[0] = 980.0;
    var->valid_range[1] = 1070.0;
  }

  else if (strcmp(name, "w") == 0) {
    set_data(var, (void **)&dumpdata->w);
    set_zloc(var, CL_CENTRE);
    var->ndims = 3;
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Vertical current");
    var->valid_range[0] = -0.1;
    var->valid_range[1] = 0.1;
  }

  else if (strcmp(name, "u1av") == 0) {
    set_data(var, (void **)&dumpdata->u1av);
    set_xyloc(var, CL_LEFT);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Easterly depth averaged current");
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }

  else if (strcmp(name, "u2av") == 0) {
    set_data(var, (void **)&dumpdata->u2av);
    set_xyloc(var, CL_BACK);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Northerly depth averaged current");
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }

  else if (strcmp(name, "u1") == 0) {
    set_data(var, (void **)&dumpdata->u1);
    set_xyloc(var, CL_LEFT);
    set_zloc(var, CL_CENTRE);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Current in e1 direction");
    var->ndims = 3;
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }
    ...
    set_data(var, (void **)&dumpdata->u2);
    set_xyloc(var, CL_BACK);
    set_zloc(var, CL_CENTRE);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Current in e2 direction");
    var->ndims = 3;
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }
    ...
    set_data_vector(var, VM_EAST, (void **)&dumpdata->u1,
                    (void **)&dumpdata->u2);
    set_xyloc_vector(var, CL_LEFT, CL_BACK);
    set_zloc_vector(var, CL_CENTRE, CL_CENTRE);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Easterly current");
    var->ndims = 3;
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;

  } else if (strcmp(name, "v") == 0) {
    set_data_vector(var, VM_NORTH, (void **)&dumpdata->u1,
                    (void **)&dumpdata->u2);
    set_xyloc_vector(var, CL_LEFT, CL_BACK);
    set_zloc_vector(var, CL_CENTRE, CL_CENTRE);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Northerly current");
    var->ndims = 3;
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }
    ...
    set_data_vector(var, VM_EAST, (void **)&dumpdata->u1av,
                    (void **)&dumpdata->u2av);
    set_xyloc_vector(var, CL_LEFT, CL_BACK);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Easterly depth averaged current");
    ...
    set_data_vector(var, VM_NORTH, (void **)&dumpdata->u1av,
                    (void **)&dumpdata->u2av);
    set_xyloc_vector(var, CL_LEFT, CL_BACK);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Northerly depth averaged current");
    ...
    set_data(var, (void **)&dumpdata->wind1);
    set_xyloc(var, CL_LEFT);
    strcpy(var->units, "Nm-2");
    strcpy(var->long_name, "Easterly wind stress");
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }

  else if (strcmp(name, "wind2") == 0) {
    set_data(var, (void **)&dumpdata->wind2);
    set_xyloc(var, CL_BACK);
    strcpy(var->units, "Nm-2");
    strcpy(var->long_name, "Northerly wind stress");
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }

  else if (strcmp(name, "u1bot") == 0) {
    set_data(var, (void **)&dumpdata->u1bot);
    set_xyloc(var, CL_LEFT);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Easterly bottom current deviation");
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }

  else if (strcmp(name, "u2bot") == 0) {
    set_data(var, (void **)&dumpdata->u2bot);
    set_xyloc(var, CL_BACK);
    strcpy(var->units, "ms-1");
    strcpy(var->long_name, "Northerly bottom current deviation");
    var->valid_range[0] = -10.0;
    var->valid_range[1] = 10.0;
  }
  else if (strcmp(name, "botz") == 0) {
    set_data(var, (void **)&dumpdata->botz);
    strcpy(var->units, "m");
    strcpy(var->long_name, "Depth level");
    var->valid_range[0] = -7000.0;
    var->valid_range[1] = 0.0;
    var->ndims = 1;
  }
  else
  ...
  if (!found) {
    ...
    for (n = 0; n < dumpdata->ntr; n++) {
      if (strcmp(dumpdata->trinfo_3d[n].name, name) == 0) {
        var->ndims = 3;
        set_data(var, (void **)&dumpdata->tr_wc[n]);
        strcpy(var->units, dumpdata->trinfo_3d[n].units);
        strcpy(var->long_name, dumpdata->trinfo_3d[n].long_name);
        var->valid_range[0] = dumpdata->trinfo_3d[n].valid_range_wc[0];
        var->valid_range[1] = dumpdata->trinfo_3d[n].valid_range_wc[1];
        var->sediment = 0;
        found = 1;
    ...
    for (n = 0; n < dumpdata->ntrS; n++) {
      if (strcmp(dumpdata->trinfo_2d[n].name, name) == 0) {
        var->ndims = 2;
        set_data(var, (void **)&dumpdata->tr_wcS[n]);
        strcpy(var->units, dumpdata->trinfo_2d[n].units);
        strcpy(var->long_name, dumpdata->trinfo_2d[n].long_name);
        var->valid_range[0] = dumpdata->trinfo_2d[n].valid_range_wc[0];
        var->valid_range[1] = dumpdata->trinfo_2d[n].valid_range_wc[1];
        var->sediment = 0;
        found = 1;
  ...
  if (!found) {
    /* Sediment tracers */
    for (n = 0; n < dumpdata->nsed; n++) {
      char sedname[MAXSTRLEN];
      strcpy(sedname, dumpdata->trinfo_sed[n].name);
      strcat(sedname, "_sed");
      if (strcmp(name, sedname) == 0) {
        var->ndims = 3;
        set_data(var, (void **)&dumpdata->tr_sed[n]);
        strcpy(var->units, dumpdata->trinfo_sed[n].units);
        strcpy(var->long_name, dumpdata->trinfo_sed[n].long_name);
        var->valid_range[0] = dumpdata->trinfo_sed[n].valid_range_sed[0];
        var->valid_range[1] = dumpdata->trinfo_sed[n].valid_range_sed[1];
        var->sediment = 1;
        found = 1;
                               ...
                               unsigned long **flag, double fi, double fj)
{
  ...
  /* Should allow for the fill_rule on dumpdata to set the values
     at land points. If a default fill_rule is used then these
     values will be NaN. If cascade_search is used then there
     should be no NaN's in the dumpfile.
  d[0] = (flag[j][i] & (SOLID | OUTSIDE)) ? NaN : values[j][i];
  d[1] = (flag[j][ip1] & (SOLID | OUTSIDE)) ? NaN : values[j][ip1];
  d[2] = (flag[jp1][ip1] & (SOLID | OUTSIDE)) ? NaN : values[jp1][ip1];
  d[3] = (flag[jp1][i] & (SOLID | OUTSIDE)) ? NaN : values[jp1][i];
  */
  if (df->filter) {
    d[0] = df_filter(df, values, flag, ne1, ne2, i, j);
    d[1] = df_filter(df, values, flag, ne1, ne2, ip1, j);
    d[2] = df_filter(df, values, flag, ne1, ne2, ip1, jp1);
    d[3] = df_filter(df, values, flag, ne1, ne2, i, jp1);
  } else {
    d[0] = values[j][i];
    d[1] = values[j][ip1];
    d[2] = values[jp1][ip1];
  ...
  /* Count the number if active neigbouring cells */
  for (n = 0; n < 4; ++n)
    if (!isnan(d[n]))
      ++nnc;

  switch (nnc) {
    /* All neighbours are NaN'd */
  case 0:
    v = NaN;
    break;

    /* Use bi-linear interpolation if all neighbours are present.  */
  case 4:
    v = (1 - di) * (1 - dj) * d[0] + di * (1 - dj) * d[1]
      + (1 - di) * dj * d[3] + di * dj * d[2];
    break;
  ...
  default:
    ...
    for (n = 0; n < 4; ++n) {
      if (!isnan(d[n])) {
        double dx = di - xcorner[n];
        double dy = dj - ycorner[n];
...
static double get_var_value_3d(dump_file_t *df, df_parray_var_t *var,
                               double ***values, int xylocation,
                               unsigned long ***flag,
                               double fi, double fj, int k)
{
  return get_var_value_2d(df, var, values[k], xylocation, flag[k], fi, fj);
}
                                  ...
                                  int vid, double **values)
{
  ...
  for (i = 0; i < df->npoints; ++i) {
    if (grid_xytofij(master->xyij_tree, df->x[i], df->y[i], &fi, &fj) >= 0) {
      v[i] = get_var_value_2d(df, var, values, get_xyloc(var),
                              dumpdata->flag[dumpdata->nz - 1], fi, fj);
    } else
      v[i] = get_missing(var);
  }
                                  ...
                                  int nz)
{
  ...
  for (i = 0; i < df->npoints; ++i) {
    if (grid_xytofij(master->xyij_tree, df->x[i], df->y[i], &fi, &fj) >= 0) {
      for (j = 0; j < nz; ++j)
        v[j][i] = get_var_value_3d(df, var, values,
                                   get_xyloc(var), dumpdata->flag, fi, fj,
                                   klower + j);
    } else {
      for (j = 0; j < nz; ++j)
        v[j][i] = get_missing(var);
    }
  }
                                  ...
                                  double **values_j)
{
  ...
  for (i = 0; i < df->npoints; ++i) {
    if (grid_xytofij(master->xyij_tree, df->x[i], df->y[i], &fi, &fj) >= 0) {
      double vi = get_var_value_2d(df, var, values_i,
                                   get_xyloc_vector(var, 0),
                                   dumpdata->flag[dumpdata->nz - 1], fi,
                                   fj);
      double vj = get_var_value_2d(df, var, values_j,
                                   get_xyloc_vector(var, 1),
                                   dumpdata->flag[dumpdata->nz - 1], fi,
                                   fj);
      v[i] =
        get_vector_component(dumpdata, vi, vj, (int)fi, (int)fj,
                             var->vector_mode);
    } else
      v[i] = get_missing(var);
  }
                                  ...
                                  double ***values_j, int klower, int nz)
{
  ...
  for (i = 0; i < df->npoints; ++i) {
    if (grid_xytofij(master->xyij_tree, df->x[i], df->y[i], &fi, &fj) >= 0) {
      for (j = 0; j < nz; ++j) {
        double vi = get_var_value_3d(df, var, values_i,
                                     get_xyloc_vector(var, 0),
                                     dumpdata->flag, fi, fj, klower + j);
        double vj = get_var_value_3d(df, var, values_j,
                                     get_xyloc_vector(var, 1),
                                     dumpdata->flag, fi, fj, klower + j);
        v[j][i] = get_vector_component(dumpdata, vi, vj, (int)fi, (int)fj,
                                       var->vector_mode);
      }
    } else {
      for (j = 0; j < nz; ++j)
...
{
  int i;
  df_memory_data_t *mem = NULL;

  df_parse_vars(dumpdata,df,PARRAY_EXCLUDE_VARS,PARRAY_ALL_VARS);
  
  mem = (df_memory_data_t *)malloc(sizeof(df_memory_data_t));
  memset(mem, 0, sizeof(df_memory_data_t));
  df->private_data = mem;
...
void df_memory_write(dump_data_t *dumpdata, dump_file_t *df, double t)
{
  int n;
  double newt = t;
  df_memory_data_t *mem = (df_memory_data_t *)df->private_data;
  ...
  */
  ...
  for (n = 0; n < df->nvars; n++) {
    ...
    if (var->ndims == 2) {
      if (var->vector_mode == VM_NONE)
        df_memory_writesub_2d(dumpdata, df, n, get_data_2d(var), v2d);
      else
        df_memory_writevec_2d(dumpdata, df, n,
                              get_data_vector_2d(var, 0),
                              get_data_vector_2d(var, 1), v2d);
      v2d += data->npoints;
    } else if (var->ndims == 3) {
      int klower = 0;
      int nz = 0;
      ...
      if (var->vector_mode == VM_NONE)
        df_memory_writesub_3d(dumpdata, df, n, get_data_3d(var),
                              klower, nz, v3d);
      else
        df_memory_writevec_3d(dumpdata, df, n,
                              get_data_vector_3d(var, 0),
                              get_data_vector_3d(var, 1), klower, nz, v3d);
                                  ...
                                  int vid, double **values, double *v2d)
{
  ...
  for (i = 0; i < df->npoints; ++i) {
    if (grid_xytofij(master->xyij_tree, df->x[i], df->y[i], &fi, &fj) >= 0) {
      *v2d = get_var_value_2d(df, var, values, get_xyloc(var),
                              dumpdata->flag[dumpdata->nz - 1], fi, fj);
    } else
      *v2d = get_missing(var);
    v2d++;
...
static void df_memory_writesub_3d(dump_data_t *dumpdata, dump_file_t *df,
                                  ...
                                  int nz, double *v3d)
{
  df_memory_data_t *data = (df_memory_data_t *)df->private_data;
  df_parray_var_t *var = &data->vars[vid];
  int i, j;
  double fi = 0.0;
  double fj = 0.0;
  ...
  for (i = 0; i < df->npoints; ++i) {
    if (grid_xytofij(master->xyij_tree, df->x[i], df->y[i], &fi, &fj) >= 0) {
      for (j = 0; j < nz; ++j) {
        *v3d = get_var_value_3d(df, var, values,
				get_xyloc(var), dumpdata->flag, fi, fj,
				klower + j);
	v3d++;
      }
    ...
    if (grid_xytofij(master->xyij_tree, df->x[i], df->y[i], &fi, &fj) >= 0) {
      for (j = 0; j < nz; ++j) {
        ...
        double vj = get_var_value_3d(df, var, values_j,
                                     get_xyloc_vector(var, 1),
                                     dumpdata->flag, fi, fj, klower + j);
        *v3d = get_vector_component(dumpdata, vi, vj, (int)fi, (int)fj,
                                       var->vector_mode);
	v3d++;
      }
    } else {
...
{
  int i, j;
  double offset = (mode) ? 180.0 : -180.0;
...
int set_transfiles(int fn, char *key, dump_data_t *dumpdata, dump_file_t *list,
  ...
  for (n = st; n <= et; n += 86400) {
	...
	if (strcmp(dumpdata->trinfo_2d[tr].name, "swr") == 0) {
      ...
      if (m)
	strcpy(buf, "eta u1mean u2mean wmean temp salt Kzmean swr");
      else
	strcpy(buf, "eta u1mean u2mean wmean temp salt Kzmean");
      if (dumpdata->tmode & INEXACT) {
	if (m)
	  strcpy(buf, "eta u1 u2 temp salt Kz origin p q r swr");
	else
	  strcpy(buf, "eta u1 u2 temp salt Kz origin p q r");
      }
      if (dumpdata->tmode & SP_FFSL) {
	if (m)
	  strcpy(buf, "eta u1mean u2mean wmean temp salt Kzmean u1vmean u2vmean swr");
	else
	  strcpy(buf, "eta u1mean u2mean wmean temp salt Kzmean u1vmean u2vmean");
      }
      list[nf].nvars = parseline(strdup(buf), list[nf].vars, MAXNUMVARS);
      strcpy(list[nf].type, "sparse");
      list[nf].create = df_sp_create;
      list[nf].write = df_sp_write;
      list[nf].close = df_sp_close;
