 ...
 *  
 *  File: model/hd/outputs/timeseries.c
 *  
 *  Description:
...
static void ts_add_point(double t, int n)
{
  ...
  i = tslist[n].i;
  j = tslist[n].j;
  c = depth2c(master, &tslist[n], tslist[n].z, tslist[n].v_offset, i, j);
  cs = geom->m2d[c];
  if (loc_ts->t != NULL) {
    // Need to recalculate these trigs for the new position
    tslist[n].sinth = (sin(geom->thetau1[cs])
		       + sin(geom->thetau1[geom->xp1[cs]])
		       + sin(geom->thetau2[cs])
		       + sin(geom->thetau2[geom->yp1[cs]])) / 4.0;
    tslist[n].costh = (cos(geom->thetau1[cs])
		       + cos(geom->thetau1[geom->xp1[cs]])
		       + cos(geom->thetau2[cs])
		       + cos(geom->thetau2[geom->yp1[cs]])) / 4.0;
  }      
  sinth = tslist[n].sinth;
  costh = tslist[n].costh;
  xp1 = geom->xp1[c];
  yp1 = geom->yp1[c];
  u1val = (master->u1[c] + master->u1[xp1]) / 2.0;
  u2val = (master->u2[c] + master->u2[yp1]) / 2.0;
  u1av =
    (master->u1av[cs] + master->u1av[geom->m2d[xp1]]) / 2.0;
  u2av =
    (master->u2av[cs] + master->u2av[geom->m2d[yp1]]) / 2.0;
  xcpt = u1val * costh - u2val * sinth;
  ycpt = u1val * sinth + u2val * costh;
  xcpta = u1av * costh - u2av * sinth;
  ycpta = u1av * sinth + u2av * costh;

  if (tslist[n].type == STANDARD) {
    xcpt = master->u1[c];
    ycpt = master->u2[c];
    xcpta = master->u1av[cs];
    ycpta = master->u2av[cs];
  }
...
static double ts_event(sched_event_t *event, double t)
{
  ...
  if (fill_all) {
    ...
    if (master->master_fill_ts)
      master->master_fill_ts(master, window, windat, wincon);
  }
  ...
  for (n = 0; n < nts; ++n) {
	...
	if (ts_has_time(loc_ts, ts_t)) {
	  ...
	  if (grid_xytoij(master->xyij_tree, 
			  tslist[n].x,  tslist[n].y, 
  ...
  for (n = 0; n < nts; ++n) {
    if (master->da && !(tslist[n].da_cycle & master->da)) continue;
    if ((tslist[n].i >= 0) && (tslist[n].tsout < tsout))
		     ...
		     geometry_t **window, dump_data_t *dumpdata)
{
  ...
  /*---------------------------------------------------------*/
    ...
    if (loc_ts->t == NULL) {
      if (grid_xytoij(master->xyij_tree, tslist[n].x, tslist[n].y, &i, &j)) {
	cs = geom->m2d[geom->map[geom->nz - 1][j][i]];
	if (cs == 0) {
	...
	if (tslist[n].v_offset == 1) {
	  ...
	  c = depth2c(master, &tslist[n], depth, 2, i, j);
	} else
	  c = depth2c(master, &tslist[n], tslist[n].z, 2, i, j);
  ...
  for (wn = 1; wn <= master->nwindows; wn++) {
    ...
    for (n = 0; n < nts; ++n) {
      int *st = NULL, ssize;
      if (tslist[n].i < 0) continue;
      c = geom->map[geom->nz - 1][tslist[n].j][tslist[n].i];
      if (c <=0 || c > geom->sgnum) continue;
      if (geom->fm[c].wn == wn) {
	ssize = stencil_size(geom, tslist[n].kernal, c);
	st = stencil(geom, c, &ssize, 0);
	for (m = 0; m < ssize; m++) {
	  c = st[m];
	  while (c != geom->zm1[c]) {
	    cc++;
  ...
  )
{
  ...
  if (mode == 0 || mode == 1)
    ...
    c = cs = geom->m2d[geom->map[geom->nz - 1][j][i]];
    if (c > 0 && c <= geom->sgnum) {
      double dc = 0.0;
      while (c != geom->zm1[c]) {
		       ...
		       )
{
  ...
  char v_name[MAXSTRLEN];    /* Observation variable name            */
  char f_name[MAXSTRLEN];    /* Observation file name                */
  geometry_t *geom = master->geom;
...
int get_glider_loc(master_t *master, ts_point_t *tslist, timeseries_t *loc_ts, double t)
{
  ...
  if (ts_has_time(loc_ts, ts_t)) {
    ...
    if (grid_xytoij(master->xyij_tree, x, y, &i, &j)) {
      c = depth2c(master, tslist, z, tslist->v_offset, i, j);
    } else
      return(0);
  } else
    return(0);
