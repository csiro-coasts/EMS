...
void usage(void)
{
  ...
  fprintf(stderr, "shoc -p prmfile [-restart] [-nrt] <options>\n");
  fprintf(stderr, "  Run SHOC using standard parameter file.\n");
  ...
  fprintf(stderr, "shoc -g prmfile dumpfile <options>\n");
  fprintf(stderr, "  Generate initial dump using standard parameter file.\n");
  fprintf(stderr, "  prmfile  :            Standard parameter file\n");
  fprintf(stderr, "  dumpfile :            Initialisation dump file\n\n");
  fprintf(stderr, "shoc -p prmfile -cr <options>\n");
  fprintf(stderr, "  Run SHOC using restarts when model crashes.\n");
  fprintf(stderr, "  prmfile  :            Standard parameter file\n\n");
  fprintf(stderr, "shoc -a prmfile <options>\n");
  fprintf(stderr, "  Run SHOC using autostart parameter file.\n");
  fprintf(stderr, "  prmfile  :            Autostart parameter file\n\n");
  fprintf(stderr, "shoc -ag prmfile <options>\n");
  fprintf(stderr, "  Generate initial dump using autostart parameter file.\n");
  fprintf(stderr, "  prmfile  :            Autostart parameter file\n\n");
  fprintf(stderr, "shoc -t prmfile <options>\n");
  fprintf(stderr, "  Run SHOC in the transport mode.\n");
  fprintf(stderr, "  prmfile  :            Transport parameter file\n\n");
  fprintf(stderr, "shoc -ps\n");
  fprintf(stderr, "  Generate percentile statistics.\n\n");
  fprintf(stderr, "shoc -v\n");
  fprintf(stderr, "  Print shoc version information.\n\n");
...
{
  return(SHOC_MAJOR_VERSION);
}

static int get_minor_vers(void)
{
  return(SHOC_MINOR_VERSION);
}

static int get_patch_vers(void)
{
  return(SHOC_PATCH_VERSION);
}
...
#ifdef HAVE_MPI
  /* Overwrite with window number */
  sprintf(timing_logfile, "timing_%d.txt", mpi_rank+1);
#endif
#endif
...
int main(int argc, char *argv[])
{
...
#ifdef HAVE_MPI
  {
    ...
    if (mpi_prov < MPI_THREAD_MULTIPLE) {
      MPI_Finalize();
      hd_warn("MPI_THREAD_MULTILE not found!\n");
      exit(1);
    }
