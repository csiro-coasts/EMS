 ...
 *  
 *  File: model/hd/include/sparse.h
 *  
 *  Description:
...
/*------------------------------------------------------------------*/
typedef struct {
  int wn;                       /* Window corresponding to global sparse
                                   cell */
  int sc;                       /* Local wet coordinate in window */
  int ac;                       /* Auxiliary coordinate in window */
} global_map_t;


typedef struct {
  int c;                        /* Sparse location of cell to interpolate */
  int cnr;                      /* South west corner coordinate (=w*_t[]) */
  int ce;                       /* Wet cell corner east of cnr */
  int cn;                       /* Wet cell corner north of cnr */
  int cne;                      /* Wet cell corner north-east of cnr */
  double x;                     /* x fractional distance of locations in
                                   cell */
  double y;                     /* y fractional distance of locations in
                                   cell */
  int cb;                       /* Global bottom sparse coordinate */
} zoom_interp_t;

typedef struct {
  int *e1bs;           /* Start blend indicies in e1 direction */
  int *e1be;           /* End blend indicies in e1 direction */
  int *e2bs;           /* Start blend indicies in e1 direction */
  int *e2be;           /* End blend indicies in e1 direction */
  int ne1b, ne2b;      /* Number of 3D blend indicies */
  int ne1bS, ne2bS;    /* Number of 2D blend indicies */
  int n2b, n3b;        /* Total number of 2D / 3D blend locations */
  int *c;              /* Blend location */
  int *p;              /* Mapping in the + direction */
  int *m;              /* Mapping in the - direction */
  int *cs;             /* Start coordinate for blending */
  int *ce;             /* End coordinate for blending */
  int *os;             /* Cell adjacent to cs[] */
  int *oe;             /* Cell adjacent to ce[] */
  int nlim, nlimS;     /* Number of cells at the end of the zone */
  int *lim;            /* End locations */
} blend_t;
...
typedef struct {
  ...
  int **be1;                    /* Boundary cells in e1 direction */
  int *nbe2;                    /* Number of boundary cells in e2 direction*/
  int **be2;                    /* Boundary cells in e2 direction */
  int *nbz;                     /* Number of boundary cells in z direction*/
...
struct win_priv {
  ...
  int u2_f;                     /* Flag to ommit u2 momentum terms */
  int u2av_f;                   /* Flag to ommit usav momentum terms */
  int orbital;                  /* Compute orbital wave speed */
  int waves;                    /* Include wave enhanced bottom friction */
  double hmean1;                /* Mean grid spacing, e1 direction (m) */
  double hmean2;                /* Mean grid spacing, e2 direction (m) */
...
struct geometry {
  ...
  int *wsa;                     /* Array of global 3D cells to process.
                                   For the */
  /* global sparse array this contains all wet */
  /* cells ordered consecutively (used to define */
  /* the window partitions). For windows this */
  ...
  /* Nested grid zoom variables */
  int zoom;                     /* Zoom mode */
  int zoomf;                    /* Zoom factor */
  int zmfe1;                    /* Zoom factor in e1 direction */  
  int zmfe2;                    /* Zoom factor in e2 direction */
  int zmme1;                    /* Zoom mapping factor in e1 direction */  
  int zmme2;                    /* Zoom mapping factor in e2 direction */
  int zmee1;                    /* Zoom edge factor in e1 direction */  
  int zmee2;                    /* Zoom edge factor in e2 direction */
  int *zoomc;                   /* Codes for centers/faces in zoom zones */
  int nzin;                     /* Number of 3D interpolation locations */
  int nzinS;                    /* Number of 2D interpolation locations */
  int *zse1;                    /* Shifting map in e1 direction */
  int *zse2;                    /* Shifting map in e2 direction */
  zoom_interp_t *zin;           /* Zoom cell sparse locations */
  int nzine1;                   /* Number of 3D interpolation locations */
  int nzine1S;                  /* Number of 2D interpolation locations */
  zoom_interp_t *zine1;         /* Zoom cell sparse locations */
  int nzine2;                   /* Number of 3D interpolation locations */
  int nzine2S;                  /* Number of 2D interpolation locations */
  zoom_interp_t *zine2;         /* Zoom cell sparse locations */
  int nipe1;                    /* Number of 3D interpolation locations */
  int nipe1S;                   /* Number of 2D interpolation locations */
  zoom_interp_t *zipe1;         /* Zoom cell sparse locations */
  int nice1;                    /* Number of 3D interpolation locations */
  int nice1S;                   /* Number of 2D interpolation locations */
  zoom_interp_t *zice1;         /* Zoom cell sparse locations */
  int nipe2;                    /* Number of 3D interpolation locations */
  int nipe2S;                   /* Number of 2D interpolation locations */
  zoom_interp_t *zipe2;         /* Zoom cell sparse locations */
  int nice2;                    /* Number of 3D interpolation locations */
  int nice2S;                   /* Number of 2D interpolation locations */
  zoom_interp_t *zice2;         /* Zoom cell sparse locations */
  int nzbe1;
  int nzbe1S;
  int *zbe1;
  int *zme1;
...
typedef struct {
  ...
  int u2_f;                     /* Flag to ommit u2 momentum terms */
  int u2av_f;                   /* Flag to ommit usav momentum terms */
  int edgef;                    /* Include edges in initialization */
  int orbital;                  /* Compute orbital wave speed */
  int waves;                    /* Include wave enhanced bottom friction */
  int rampf;                    /* Variables to ramp flag */
  int totals;                   /* Plots integrated totals */
  ...
  int noutside;                 /* Number of cells to redefine as outside */
  int *oute1;                   /* e1 list of defined outside cells */
  int *oute2;                   /* e2 list of defined outside cells */
...
struct master {
  ...
  /* Distributed processing and transfer buffers mode bitfield */
  int dp_mode;
  ...
  double *u1m;                  /* Pointer to u1 mean velocity */
  double *u2m;                  /* Pointer to u2 mean velocity */
  ...
  double *u1am;                 /* Pointer to u1av mean velocity */
  double *u2am;                 /* Pointer to u2av mean velocity */
  double *w1m;                  /* Pointer to e1 mean wind */
  double *w2m;                  /* Pointer to e2 mean wind */
  ...
  double *speedd_2d;            /* 2D current direction */
  double *speedd_3d;            /* 3D current direction */
  ...
  /* Nested grid zoom variables */
  int dozoom;                   /* Zoom flag */
  int zoomf;                    /* Zoom flag; same as dozoom */
  int zmfe1;                    /* Zoom factor in e1 direction */  
  int zmfe2;                    /* Zoom factor in e2 direction */
  double *u1flux_z;             /* 2D zoom e1 fluxes for m2s transfers */
  double *u2flux_z;             /* 2D zoom e2 fluxes for m2s transfers */
...
struct window {
  ...
  double *u1;                   /* e1 3D velocity (ms-1) */
  double *u2;                   /* e2 3D velocity (ms-1) */
  double *nu1;                  /* Updated 3D e1 velocity (ms-1) */
  ...
  /* pointer to buffered object */
  void *win_data_buffered_obj;
};
