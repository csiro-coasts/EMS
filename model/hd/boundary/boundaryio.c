 ...
 *  
 *  File: model/hd/boundary/boundaryio.c
 *  
 *  Description:
...
void *tsfiles_alloc(master_t *master, bdry_details_t *data);
		   ...
		   )
{
  ...
  if (strcasecmp(buf, "u1") == 0) {
    ...
    open->type |= U2BDRY;
  } else if (strcasecmp(buf, "velocity") == 0) {
    open->type |= (U1BDRY | U2BDRY);
  } else
    hd_quit
  ...
  case U1BDRY:
    ...
    if (prm_read_char(fp, keyword, open->cusname_u1) && bdf)
      hd_warn
        ("Multiple custom routines (eta and u1) specified for BOUNDARY%1d.\n",
         n);
    sprintf(keyword, "BOUNDARY%1d.CUSTOM.%s", n, "u2");
    if (prm_read_char(fp, keyword, open->cusname_u2) && bdf)
      hd_quit
        ("Multiple custom routines (eta and u2) specified for BOUNDARY%1d.\n",
         n);
  ...
  case U2BDRY:
    ...
    sprintf(keyword, "BOUNDARY%1d.CUSTOM.%s", n, "u1av");
    prm_read_char(fp, keyword, open->cusname_u1av);
    if (strcmp(open->cusname_u2, "u2flowbdry") == 0 || strlen(open->bstd[0])) {
      sprintf(keyword, "BOUNDARY%1d.U2_FLOW", n);
      prm_read_char(fp, keyword, open->bflow);
      sprintf(keyword, "BOUNDARY%1d.U2_HC", n);
      prm_read_double(fp, keyword, &open->bhc);
      sprintf(keyword, "BOUNDARY%1d.U2_LENGTH", n);
      prm_read_double(fp, keyword, &open->rlen);
    }
    break;
  case (U1BDRY | U2BDRY):
  ...
  )
{
  ...
  open->ntr = io->ntr;
  open->atr = io->atr;
  /*UR-FIX assign extra element  bcond_tra - otherwise read from 
   * uninitialised value
   *org 
   * open->bcond_tra = i_alloc_1d(open->ntr);
   */
  ...
  )
{
  ...
  else if (open->type & U2BDRY && strcmp(name, "u1") == 0 && 
	   open->bcond_tan & (FILEIN | CUSTOM)) {       /* u1 TAN    */
    strcpy(data->name, "u1");
    data->type = TAN;
  }
  else if (open->type & U2BDRY && strcmp(name, "u2") == 0 && 
	   open->bcond_nor & (FILEIN | CUSTOM)) {       /* u2 NOR    */
    strcpy(data->name, "u2");
    data->type = NOR;
  }
  ...
  else if (open->type & U2BDRY && strcmp(name, "u1av") == 0 && 
	   open->bcond_tan2d & (FILEIN | CUSTOM)) {     /* u1av TAN  */
    strcpy(data->name, "u1av");
    data->type = TAN;
  }
  else if (open->type & U2BDRY && strcmp(name, "u2av") == 0 && 
	   open->bcond_nor2d & (FILEIN | CUSTOM)) {     /* u2av NOR  */
  ...
  if (strlen(cusname)) {
    ...
    locate_boundary_function(open, fields[0], &data->init_m, &data->init_w,
                             &data->custom_m, &data->custom_mw, &data->custom_w,
...
void bdry_data_copy(bdry_details_t *data, bdry_details_t *din)
{
  ...
  if (data->nargs) {
    ...
    /* Only copy file names */
    if (strlen(data->custom_tag) == 0)
      data->custdata = (void *)tsfiles_alloc(master, din);
  }
  /* Set the custom function for the window the same as the master */
  data->custom_m = din->custom_m;
			      ...
			      bdryfree_m * free_m, bdryfree_w * free_w )
{
  ...
  static bdryfn_map_t bdryfn_standard[] = {
    {"hdstd_to_u1", bf_hdstd_to_u1_init_m, bf_hdstd_to_u1_init_w,
     bf_hdstd_to_u1_m, NULL, bf_hdstd_to_u1_w, bf_hdstd_to_u1_t,
     bf_ts_free, bf_void_free},
    {"hdstd_to_u2", bf_hdstd_to_u2_init_m, bf_hdstd_to_u2_init_w,
     bf_hdstd_to_u2_m, NULL, bf_hdstd_to_u2_w, bf_hdstd_to_u2_t,
     bf_ts_free, bf_void_free},

    {"hdstd_to_u1av", bf_hdstd_to_u1_init_m, bf_uv_to_uav_init_w,
     bf_hdstd_to_u1av_m, NULL, bf_uv_to_u1av_w, bf_uvav_to_u1av_t,
     bf_ts_free, bf_void_free},
    {"hdstd_to_u2av", bf_hdstd_to_u2_init_m, bf_uv_to_uav_init_w,
     bf_hdstd_to_u2av_m, NULL, bf_uv_to_u2av_w, bf_uvav_to_u2av_t,
     bf_ts_free, bf_void_free},

    {"uv_to_u1", bf_uv_to_u1_init_m, bf_uv_to_u1_init_w,
     bf_uv_to_u1_m, bf_uv_to_u1_mw, bf_uv_to_u1_w, bf_uv_to_u1_t,
     bf_ts_free, bf_void_free},
    {"uv_to_u2", bf_uv_to_u2_init_m, bf_uv_to_u2_init_w,
     bf_uv_to_u2_m, bf_uv_to_u2_mw, bf_uv_to_u2_w, bf_uv_to_u2_t,
     bf_ts_free, bf_void_free},

    {"uv_to_u1av", bf_uv_to_u1_init_m, bf_uv_to_uav_init_w,
     bf_uv_to_u1av_m, NULL, bf_uv_to_u1av_w, bf_uvav_to_u1av_t,
     bf_ts_free, bf_c2cc_free},
    {"uv_to_u2av", bf_uv_to_u2_init_m, bf_uv_to_uav_init_w,
     bf_uv_to_u2av_m, NULL, bf_uv_to_u2av_w, bf_uvav_to_u2av_t,
     bf_ts_free, bf_c2cc_free},

    {"uvav_to_u1av", bf_uvav_to_u1av_init_m, bf_uvav_to_u1av_init_w,
     bf_uvav_to_u1av_m, NULL, bf_uvav_to_u1av_w, bf_uvav_to_u1av_t},
    {"uvav_to_u2av", bf_uvav_to_u2av_init_m, bf_uvav_to_u2av_init_w,
     bf_uvav_to_u2av_m, NULL, bf_uvav_to_u2av_w, bf_uvav_to_u2av_t,
     bf_ts_free, bf_void_free},
    /*
    ...
    */
    {"u1flowbdry", bf_u1_flow_init_m, bf_u1_flow_init_w,
     bf_flow_m, NULL, bf_u1_flow_w, bf_flow_t,
     bf_flow_free, bf_void_free},
    {"u2flowbdry", bf_u2_flow_init_m, bf_u2_flow_init_w,
     bf_flow_m, NULL, bf_u2_flow_w, bf_flow_t,
     bf_flow_free, bf_void_free},
    {"use_eqn", bf_use_eqn_init_m, NULL, bf_use_eqn_m, NULL, 
     bf_use_eqn_w, bf_use_eqn_trans, bf_eqn_free, bf_void_free},
  ...
  while (bdryfn_standard[i].name != NULL) {
    if (strcmp(bdryfn_standard[i].name, tag) == 0) {
      ...
      *func_mw = bdryfn_standard[i].func_mw;
...
/*-------------------------------------------------------------------*/
/* Routine to read in boundary cell lists                            */
/*-------------------------------------------------------------------*/
void get_obc_list(open_bdrys_t *open, FILE *fp, int n, char *key) {
  int nn;
  int is, ie, js, je;
  char key1[MAXSTRLEN], key2[MAXSTRLEN], buf[MAXSTRLEN];
  ...
  if (prm_read_int(fp, key1, &open->npts)) {
    ...
    if (open->type & U1BDRY) {
      if (is != ie) 
	hd_quit("params_read: start and end e1 coordinates must be the same for u1 boundaries %d (%d != %d).\n", n, is, ie);
      else if (je-js+1 <= 0)
	hd_quit("params_read: start > end e2 coordinates for u1 boundaries %d (%d != %d).\n", n, js, je);
      else {
	open->npts = je - js + 1;
	open->iloc = i_alloc_1d(open->npts);
	open->jloc = i_alloc_1d(open->npts);
	for (nn = 0; nn < open->npts; nn++) {
      ...
      if (js != je) 
	hd_quit("params_read: start and end e2 coordinates must be the same for u2 boundaries %d (%d != %d).\n",n, js, je);
      else if (ie-is+1 <= 0)
	hd_quit("params_read: start > end e2 coordinates for u2 boundaries %d (%d != %d).\n", n, is, ie);
      else {
	open->npts = ie - is + 1;
	open->iloc = i_alloc_1d(open->npts);
	open->jloc = i_alloc_1d(open->npts);
	for (nn = 0; nn < open->npts; nn++) {
      ...
      hd_quit("params_read: cannot use %s%d.TYPE %d with RANGE.\n", key, n, open->type);
    }
  } else {
    hd_quit("params_read: cannot find %s%d (%s) POINTS or RANGE.\n", key, n, open->name);
  }
  ...
  if (prm_read_char(fp, key1, buf)) {
    if (open->type & U1BDRY) {
      if (is != ie) 
	hd_quit("params_read: start and end e1 coordinates must be the same for cycled u1 boundaries %d (%d != %d).\n", n, is, ie);
      else if (je-js+1 <= 0)
	hd_quit("params_read: start > end e2 coordinates for cycled u1 boundaries %d (%d != %d).\n", n, js, je);
      else {
	sscanf(buf, "(%d,%d)-(%d,%d)", &is, &js, &ie, &je);
	open->ncyc = je - js + 1;
	open->ilocc = i_alloc_1d(open->ncyc);
	...
	for (nn = 0; nn < open->ncyc; nn++) {
      ...
      if (js != je) 
	hd_quit("params_read: start and end e2 coordinates must be the same for cycled u2 boundaries %d (%d != %d).\n",n, js, je);
      else if (ie-is+1 <= 0)
	hd_quit("params_read: start > end e2 coordinates for cycled u2 boundaries %d (%d != %d).\n", n, is, ie);
      else {
	open->ncyc = ie - is + 1;
	open->ilocc = i_alloc_1d(open->ncyc);
	open->jlocc = i_alloc_1d(open->ncyc);
	      ...
	      int mode)
{
  ...
  if (mode & NEST1WAY) {
    ...
    */
    ...
    if (open->type & U2BDRY) {
      strcpy(cusu1, "uv_to_u2");
      strcpy(cusu2, "uv_to_u1");
      if (mode & NEST_STD) {
	strcpy(cusu1, "hdstd_to_u2");
	strcpy(cusu2, "hdstd_to_u1");
      }
      sprintf(open->cusname_u2, "%s %s", cusu1, file[nfiles-2]);
      sprintf(open->cusname_u1, "%s %s", cusu2, file[nfiles-1]);
    }
  }
  ...
  if (mode & NEST2WAY) {
    ...
    */
	...
	strcpy(cusu2, "hdstd_to_u1");
      ...
      sprintf(open->cusname_u2, "%s %s_uv_nor.mpk", cusu1, file[nfiles-2]);
      sprintf(open->cusname_u1, "%s %s_uv_tan.mpk", cusu2, file[nfiles-1]);
      if (do_baro) {
	strcpy(cusu1, "uvav_to_u2av");
	strcpy(cusu2, "uvav_to_u1av");
	if (mode & NEST_STD) {
	  strcpy(cusu1, "hdstd_to_u2av");
	  strcpy(cusu2, "hdstd_to_u1av");
	}
	sprintf(open->cusname_u2av, "%s %s_uvav_nor.mpk", cusu1, file[nfiles-2]);
	sprintf(open->cusname_u1av, "%s %s_uvav_tan.mpk", cusu2, file[nfiles-1]);
      } else {
	open->bcond_nor2d = VERTIN;
	open->bcond_tan2d = VERTIN;
      }
  ...
  if (mode & NEST_CPD) {
	...
	if (suff) {
	  ...
	  sprintf(open->cusname_u2av, "%s %s_uv_nor.mpk", cusu1, file[nfiles-2]);
	  sprintf(open->cusname_u1av, "%s %s_uv_tan.mpk", cusu2, file[nfiles-1]);
	}
      } else {
	open->bcond_nor2d = VERTIN;
	open->bcond_tan2d = VERTIN;
  ...
  if (mode & NEST_FLA) {
    ...
    /* Sea level                                                     */
    open->bcond_ele = FLATHR|FILEIN|GRAVTY;
    ...
    if (open->type & U2BDRY) {
      ...
      if (mode & NEST_STD)
	strcpy(cusu2, "hdstd_to_u2av");
      sprintf(open->cusname_u2av, "%s %s", cusu2, file[nfiles-2]);
      if (velf) {
	strcpy(cusu2, "uv_to_u2");
	sprintf(open->cusname_u2, "%s %s", cusu2, file[nfiles-2]);
	strcpy(cusu1, "uv_to_u1");
	sprintf(open->cusname_u1, "%s %s", cusu1, file[nfiles-1]);
      }
    }
  }
  ...
  if (mode & RIVER) {
    ...
    if (open->type & U2BDRY) {
      sprintf(open->cusname_u2, "u2flowbdry");
    }
