  ...
  )
{
  ...
  for (cc = 1; cc <= window->a2_t; cc++) {
    ...
    zm1 = window->zm1[c];
    while (c < cb) {
      vu1[c] = windat->u1[c] - windat->u1[zm1];
      vu2[c] = windat->u2[c] - windat->u2[zm1];
      c = zm1;
      zm1 = window->zm1[c];
    }
  }
  ...
  for (cc = 1; cc <= window->b2_t; cc++) {
    ...
    while (c < cb) {
      ...
      xp1 = window->xp1[c];
      du1 = 0.5 * (vu1[c] + vu1[xp1]);

      /* Vertical u2 gradient */
      yp1 = window->yp1[c];
      du2 = 0.5 * (vu2[c] + vu2[yp1]);
	...
	windat->s_prod[c] = windat->Vz[c] * (du1dz * du1dz + du2dz * du2dz);
	  ...
	  /*
          windat->Vz[c] = 0.5 * (windat->Vz[window->xm1[c]] +
			         windat->Vz[window->ym1[c]]);
          windat->Kz[c] = 0.5 * (windat->Kz[window->xm1[c]] +
				 windat->Kz[window->ym1[c]]);
	  */
        }
      } else {
        windat->Vz[c] = wincon->vz0;
