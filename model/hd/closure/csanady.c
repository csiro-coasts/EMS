 ...
 *  
 *  File: model/hd/closure/csanady.c
 *  
 *  Description:
 ...
 *  
 *  $Id: csanady.c 5841 2018-06-28 06:51:55Z riz008 $
 *
 */
  ...
  )
{
  ...
  for (cc = 1; cc <= window->b3_t; cc++) {
    ...
    double u1val = 0.5 * (windat->u1[c] + windat->u1[xp1]);
    double u2val = 0.5 * (windat->u2[c] + windat->u2[yp1]);
