 ...
 *  
 *  File: model/hd/closure/k-e-implicit.c
 *  
 *  Description:
 ...
 *  
 *  $Id: k-e-implicit.c 5841 2018-06-28 06:51:55Z riz008 $
 *
 */
...
/*-------------------------------------------------------------------*/
void closure_k_e_implicit(geometry_t *window, /* Processing window */
  ...
  )
{
  ...
  /* Vertical u1 and u2 gradients */
  for (cc = 1; cc <= window->a2_t; cc++) {
    c = window->w2_t[cc];
    cb = window->bot_t[cc];
    zm1 = window->zm1[c];
    while (c < cb) {
      tke[c] = windat->u1[c] - windat->u1[zm1];
      diss[c] = windat->u2[c] - windat->u2[zm1];
      c = zm1;
      zm1 = window->zm1[c];
    }
  }

  /* Calculate shear at layer interfaces */
  for (cc = 1; cc <= window->b3_t; cc++) {
    ...
    du1[c] = 0.5 * (tke[c] + tke[xp1]);

    /* Vertical u2 gradient */
    du2[c] = 0.5 * (diss[c] + diss[yp1]);
  }
  ...
  for (cc = 1; cc <= vcs; cc++) {
    ...
    if (zm1 == window->zm1[zm1]) {
      k_e_1layer(window, windat, wincon, c, c2);
      continue;
    }
	...
	v1 = 0.5 * (windat->wind1[c2] + windat->wind1[window->xp1[c2]]);
	v2 = 0.5 * (windat->wind2[c2] + windat->wind2[window->yp1[c2]]);
	wflux = sqrt(sqrt(v1 * v1 + v2 * v2) / RHO_0);
