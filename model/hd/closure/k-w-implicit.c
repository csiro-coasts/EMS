 ...
 *  
 *  File: model/hd/closure/k-w-implicit.c
 *  
 *  Description:
 ...
 *  
 *  $Id: k-w-implicit.c 5841 2018-06-28 06:51:55Z riz008 $
 *
 */
...
/*-------------------------------------------------------------------*/
void closure_k_w_implicit(geometry_t *window, /* Processing window */
  ...
  )
{
  ...
  for (cc = 1; cc <= vcs; cc++) {
    ...
    if (zm1 == window->zm1[zm1]) {
      k_w_1layer(window, windat, wincon, c, cs);
      continue;
    }
	...
	du1 = 0.5 * (windat->wind1[cs] + windat->wind1[window->xp1[cs]]);
	du2 = 0.5 * (windat->wind2[cs] + windat->wind2[window->yp1[cs]]);
	wflux = sqrt(sqrt(du1 * du1 + du2 * du2) / RHO_0);
  ...
  )
{
  ...
  /* Surface stress */
  v1 = 0.5 * (windat->wind1[cs] + windat->wind1[window->xp1[cs]]);
  v2 = 0.5 * (windat->wind2[cs] + windat->wind2[window->yp1[cs]]);
  ustr_surf = sqrt(sqrt(v1 * v1 + v2 * v2) / RHO_0);

  /* Bottom stress */
  v1 = 0.5 * (windat->u1[c] + windat->u1[xp1]);
  v2 = 0.5 * (windat->u2[c] + windat->u2[yp1]);
  ustr_bot = sqrt(wincon->Cd[cs] * (v1 * v1 + v2 * v2));
...
{
  int c, cc, c2;                /* Sparse coordinates */
  ...
  for (cc = 1; cc <= window->b3_t; cc++) {
    ...
    p1m = window->zm1[p1];
    m1m = window->zm1[m1];
    chik[c] = (0.5 * (tke[p1] - tke[m1] + tke[p1m] - tke[m1m]) *
               0.5 * (omega[p1] - omega[m1] + omega[p1m] - omega[m1m])) /
      (4.0 * window->h1acell[c2] * window->h1acell[c2]);
    p1 = window->yp1[c];
    m1 = window->ym1[c];
    chik[c] += (0.5 * (tke[p1] - tke[m1] + tke[p1m] - tke[m1m]) *
                0.5 * (omega[p1] - omega[m1] + omega[p1m] - omega[m1m])) /
      (4.0 * window->h2acell[c2] * window->h2acell[c2]);
    m1 = window->zm1[c];
