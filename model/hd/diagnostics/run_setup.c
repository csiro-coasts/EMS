 ...
 *  
 *  File: model/hd/diagnostics/run_setup.c
 *  
 *  Description:
...
void write_run_setup(hd_data_t *hd_data)
{
  ...
  fprintf(fp, "SHOC Simulation Summary\n");
  ...
  for (cc = 1; cc <= geom->b2_t; cc++) {
    c = geom->w2_t[cc];
    d1 = 1.0 / (geom->h1acell[c] * geom->h1acell[c]) +
  ...
  if (params->nwindows > 1) {
    ...
    if (params->dozoom) {
      FILE *op = fopen("zoom.points", "w");
      fprintf(fp, " Grid refinement performed : zoom factor = %d\n\n",
	      params->zoomf);
      for (n = 1; n <= params->nzoom; n++) {
	int i = params->zci[n];
	int j = params->zcj[n];
	c = geom->map[geom->nz-1][j][i];
	fprintf(op, "p%d %f %f 0.0\n",n, geom->cellx[c], geom->celly[c]);
      }
      fclose(op);
    }
    fprintf(fp, "\n");
...
#if defined(HAVE_OMP)
  if (master->dp_mode & DP_OPENMP)
    fprintf(fp, "Distributed processing : OpenMP\n");
#endif
#if defined(HAVE_MPI)
  if (master->dp_mode & DP_MPI) {
    int mpi_vers, mpi_subvers;
    MPI_Get_version(&mpi_vers, &mpi_subvers);
    fprintf(fp, "Distributed processing : Message Passing Interface (MPI) v%d.%d\n", mpi_vers, mpi_subvers);
  }
#endif
#if defined(HAVE_PTHREADS)
  if (master->dp_mode & DP_PTHREADS)
  ...
  if (master->nbl1) {
    for (n = 0; n < master->nbl1; n++)
      fprintf(fp, "Blending e1 velocity from j=%d to j=%d, between i = %d & %d.\n", 
	      master->e1bs[n], master->e1be[n], master->e2bs[n], master->e2be[n]);
  }
  if (master->nbl2) {
    for (n = 0; n < master->nbl2; n++)
      fprintf(fp, "Blending e2 velocity from i=%d to j=%d, between j = %d & %d.\n", 
	      master->e2bs[n], master->e2be[n], master->e1bs[n], master->e1be[n]);
  }
  ...
  if (!(params->u2_f & NONE)) {
    fprintf(fp, "u2 momentum terms omitted :\n");
    if (params->u2_f & ADVECT)
      fprintf(fp, "  ADVECT\n");
    if (params->u2_f & HDIFF)
      fprintf(fp, "  HORIZONTAL DIFFUSION\n");
    if (params->u2_f & VDIFF)
      fprintf(fp, "  VERTICAL DIFFUSION\n");
    if (params->u2_f & CORIOLIS)
      fprintf(fp, "  CORIOLIS\n");
    if (params->u2_f & PRESS_BT)
      fprintf(fp, "  BAROTROPIC PRESSURE\n");
    if (params->u2_f & PRESS_BC)
      fprintf(fp, "  BAROCLINIC PRESSURE\n");
  }
  ...
  if (!(params->u2av_f & NONE)) {
    fprintf(fp, "u2av momentum terms omitted :\n");
    if (params->u2av_f & ADVECT)
      fprintf(fp, "  ADVECT\n");
    if (params->u2av_f & HDIFF)
      fprintf(fp, "  HORIZONTAL DIFFUSION\n");
    if (params->u2av_f & VDIFF)
      fprintf(fp, "  VERTICAL DIFFUSION\n");
    if (params->u2av_f & CORIOLIS)
      fprintf(fp, "  CORIOLIS\n");
    if (params->u2av_f & PRESS_BT)
      fprintf(fp, "  BAROTROPIC PRESSURE\n");
    if (params->u2av_f & PRESS_BC)
      fprintf(fp, "  BAROCLINIC PRESSURE\n");
  }
  ...
  if (params->momsc & LAGRANGE) {
    ...
    if (params->momsc & ANGULAR)
      fprintf(fp, "2nd order semi-implcit angular momentum advection scheme.\n");
    if (params->momsc & ANGULAR3D)
      fprintf(fp, "2nd order momentum advection scheme : angular on 3d mode.\n");
    if (params->momsc & (ANGULAR|EXPLICIT))
      fprintf(fp, "2nd order explicit angular momentum advection scheme.\n");
  }
  ...
  if (params->trasc == ORIGINAL)
    fprintf(fp, "1st order upwind (original) tracer advection scheme.\n");
  ...
  if (params->trasc == QUICKEST_AD)
    fprintf(fp, "QUICKEST (advective form) tracer advection scheme.\n");
  ...
  if (params->trasc == QUICKEST_CO)
    fprintf(fp,
            "QUICKEST (flux form : constant grid) tracer advection scheme.\n");
  if (params->trasc == VANLEER)
    fprintf(fp, "Van Leer tracer advection scheme.\n");
  ...
  if (params->u1kh < 0.0 && params->smagorinsky != 0.0) {
    ...
    fprintf(fp,
            "Smagorinsky horizontal diffusion (x direction); constant = %5.3f\n", smag);
    if (params->bkue1) fprintf(fp,"  base rate applied = %5.3f\n", params->bkue1);
  } else if (params->u1kh >= 0.0 && params->diff_scale == AUTO)
    fprintf(fp,"Grid-optimized horizontal diffusion (x direction)\n");
  else {
    if (params->diff_scale == LINEAR)
      fprintf(fp, "Horizontal diffusion (x direction) = %5.3f : linearly scaled\n",
            params->u1kh);
    else if (params->diff_scale == NONLIN)
      fprintf(fp, "Horizontal diffusion (x direction) = %5.3f : non-linearly scaled\n",
            params->u1kh);
    else if (params->diff_scale == NONE)
      fprintf(fp, "Horizontal diffusion (x direction) = %5.3f : un-scaled\n",
            params->u1kh);
  }
  if (params->u2kh < 0.0 && params->smagorinsky != 0.0) {
    double smag = (params->smagorinsky == 1 && params->kue2!= 1) ? params->kue2 : params->smagorinsky;
    fprintf(fp,
            "Smagorinsky horizontal diffusion (y direction); constant = %5.3f\n", smag);
    if (params->bkue2) fprintf(fp,"  base rate applied = %5.3f\n", params->bkue2);
  } else if (params->u2kh >= 0.0 && params->diff_scale == AUTO)
    fprintf(fp,"Grid-optimized horizontal diffusion (y direction)\n");
  else {
    if (params->diff_scale == LINEAR)
      fprintf(fp, "Horizontal diffusion (y direction) = %5.3f : linearly scaled\n",
            params->u2kh);
    else if (params->diff_scale == NONLIN)
      fprintf(fp, "Horizontal diffusion (y direction) = %5.3f : non-linearly scaled\n",
            params->u2kh);
    else if (params->diff_scale == NONE)
      fprintf(fp, "Horizontal diffusion (y direction) = %5.3f : un-scaled\n",
            params->u2kh);
  }
  if (params->u1vh < 0.0 && params->smagorinsky != 0.0) {
    double smag = (params->smagorinsky == 1 && params->sue1 != 1) ? params->sue1 : params->smagorinsky;
    fprintf(fp,
            "Smagorinsky horizontal viscosity (x direction); constant = %5.3f\n", smag);
    if (params->bsue1) fprintf(fp,"  base rate applied = %5.3f\n", params->bsue1);
  } else if (params->u1vh >= 0.0 && params->diff_scale == AUTO)
    fprintf(fp,"Grid-optimized horizontal viscosity (x direction)\n");
  else {
    if (params->diff_scale == LINEAR)
      fprintf(fp, "Horizontal viscosity (x direction) = %5.3f : linearly scaled\n",
            params->u1vh);
    else if (params->diff_scale == NONLIN)
      fprintf(fp, "Horizontal viscosity (x direction) = %5.3f : non-linearly scaled\n",
            params->u1vh);
    else if (params->diff_scale == NONE)
      fprintf(fp, "Horizontal viscosity (x direction) = %5.3f : un-scaled\n",
            params->u1vh);
  }
  if (params->u2vh < 0.0 && params->smagorinsky != 0.0) {
    double smag = (params->smagorinsky == 1 && params->sue2 != 1) ? params->sue2 : params->smagorinsky;
    fprintf(fp,
            "Smagorinsky horizontal viscosity (y direction); constant = %5.3f\n", smag);
    if (params->bsue2) fprintf(fp,"  base rate applied = %5.3f\n", params->bsue2);
  } else if (params->u2vh >= 0.0 && params->diff_scale == AUTO)
    fprintf(fp,"Grid-optimized horizontal viscosity (y direction)\n");
  else {
    if (params->diff_scale == LINEAR)
      fprintf(fp, "Horizontal viscosity (y direction) = %5.3f : linearly scaled\n",
            params->u2vh);
    else if (params->diff_scale == NONLIN)
      fprintf(fp, "Horizontal viscosity (y direction) = %5.3f : non-linearly scaled\n",
            params->u2vh);
    else if (params->diff_scale == NONE)
      fprintf(fp, "Horizontal viscosity (y direction) = %5.3f : un-scaled\n",
            params->u2vh);
  }
  ...
  fprintf(fp, "Mean horizontal grid spacing : e1 = %8.2f, e2 = %8.2f\n",
          master->hmean1, master->hmean2);
  ...
  if (geom->nobc) {
	...
	if (open->relax_time != open->relax_timei) {
      ...
      if (open->sponge_zone_h) {
        c = open->obc_t[1];
        fprintf(fp, "    Horizontal sponge zone : e1=%6.3f, e2=%6.3f\n",
                master->u1vh[c], master->u2vh[c]);
      }
	...
	if (params->compatible & V1670)
	  ...
	  if (open->bcond_ele & TIDALC)
	    fprintf(fp, "      eta tide specified using custom tide\n");
	  if (open->bcond_ele & TIDALH)
	    fprintf(fp, "      eta tide specified using CSR tide\n");
	}
      }
  ...
  /*
  if(master->npss) {
    fprintf(fp,"Number of source/sinks =%d\n",master->npss);
    for(nn=0; nn<master->npss; nn++) {
      pss_t *pss = &master->pss[nn];
      fprintf(fp," Source/sink #%d : %s\n",nn,pss->name);
      fprintf(fp," Location %d (i,j,k) = (%d,%d,%d)\n",pss->e1,
	      geom->s2i[pss->e1],geom->s2j[pss->e1],geom->s2k[pss->e1]);
    }
    fprintf(fp,"\n");
  }
  */
  if (master->npss) {
	...
	for (cc = 0; cc < p->vc; cc++) {
	  ...
	  if (pdz > 0.0) {
		...
		zhigh = zhigh_i;
	      ...
	      if (frac) {
		d1 += geom->cellarea[c2];
		d2 += geom->h2au1[c2] * frac;
		d3 += geom->h1au2[c2] * frac;
	      }
	      zp1 = c;
	      c = geom->zm1[c];
	    }
	...
	fprintf(fp,"  e1 planar area = %8.4f km^2\n", d2/1e6);
	fprintf(fp,"  e2 planar area = %8.4f km^2\n", d3/1e6);
  ...
  if (params->waves & STOKES_WIND && master->tau_w1 && master->tau_diss1 && master->tau_w2 && master->tau_diss2)
  ...
  if (geom->nregions) {
	...
	ncells = (region->nbz[region->bmap[nn]] ? region->nbz[region->bmap[nn]] :
		  region->nbe1[region->bmap[nn]] + region->nbe2[region->bmap[nn]]);
