 ...
 *  
 *  $Id: dp.c 5873 2018-07-06 07:23:48Z riz008 $
 *
 */
...
#endif
#if defined(HAVE_MPI)
  if (strcasecmp(master->params->dp_mode, "mpithreads") == 0) {
    dp.init = dp_mpi_init;
...
#else
  if (strcasecmp(master->params->dp_mode, "mpithreads") == 0) {
    hd_quit("DP_MODE of MPITHREADS has been specified but this executable has not been built with MPI support. Please reconfigure and rebuild and try again.");
  } else    
#endif
#if defined(HAVE_OMP)
...
#else
  if (strcasecmp(master->params->dp_mode, "openmp") == 0) {
    hd_quit("DP_MODE of OPENMP has been specified but this executable has not been built with OpenMP support. Please reconfigure and rebuild and try again.");
  } else
#endif
  {
#if defined(HAVE_OMP)
     ...
     * in which case, we setup for sequential mode
     */
