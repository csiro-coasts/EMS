...
void read_bathy_from_sparse_nc(parameters_t *params, char *fname)
{
  ...
  if (spf) {
    ...
    nc_inq_varndims(fid, ncw_dim_id(fid, bathy_dims[intype][3]), &lond);
    nc_inq_varndims(fid, ncw_dim_id(fid, bathy_dims[intype][4]), &latd);
...
void read_grid(parameters_t *params)
{
  ...
  if (prm_read_int(fp, "nMaxMesh2_face_nodes", &params->npe) &&
    ...
    /* Create the mesh                                               */
    create_jigsaw_mesh(cm, &J_mesh, &J_hfun, powf, stproj);
