 ...
 *  
 *  File: model/hd-us/tracers/regions.c
 *  
 *  Description:
 *  
 *  Copyright:
...
void get_regions(geometry_t *win, region_t *region, double *mask, int nregions)
{
  ...
  if (rn) {
	...
	rn = (int)mask[cn];
	if (rn != NOTVALID && n != rn) {
	  region->sgne1[rn][region->nbe1[rn]] = -win->eSc[j][c];
	  region->be1[rn][region->nbe1[rn]++] = e;
	}
      }
    }
    for (m = 0; m < win->nobc; m++) {
	...
	for (ee= 1; ee <= open->no3_e2; ee++) {
	  ...
	  region->sgne1[m + nregions][region->nbe1[m + nregions]] = -win->eSc[j][c];
	  if (region->mode & RG_BDRY)
	    for (bb = 0; bb < region->obcz; bb++)
	      c = open->nmape[ee][c];
	  cr = win->c2e[j][c];
  ...
  for (m = 0; m < region->nfaces; m++) {
    ...
    if (region->mode & RG_ALLT) {
      ...
      if (m != region->id  && (region->nbe1[m] || region->nbz[m])) {
	region->bmap[region->nboundaries] = m;
	region->rmap[m] = region->nboundaries;
	region->nboundaries++;
      }
		      ...
		      double *Fx, double *Fz, double dt, int trn)
{
  ...
  for (n = 0; n < window->nregions; n++) {
    ...
    for (bb = 0; bb < region->nboundaries; bb++) {
      bn = region->bmap[bb];
      if (region->nbe1[bn]) {
	for (ee = 0; ee < region->nbe1[bn]; ee++) {
	  e = region->be1[bn][ee];
	  region->trflux[tt][bb] += (Fx[e] * dt * region->sgne1[bn][ee]);
	}
      }
      if (region->nbz[bn]) {
			     ...
			     double dt, double trem)
{
  ...
  for (n = 0; n < window->nregions; n++) {
	...
	for (ee = 0; ee < region->nbe1[bn]; ee++) {
	  e = region->be1[bn][ee];
	  flux = (windat->u1flux3d[e] * dt * region->sgne1[bn][ee]);
...
int read_regioni(master_t *master, char *rname, double *regionid)
{
  ...
  /* nregions = 1, even though no regions are contained in the       */
  /* calling geographic domain. This manifests if the coordinate     */
  /* system of the model is in m, but the region file is lat/long.   */
  nregions = 0;
  for (cc = 1; cc <= geom->b3_t; cc++) {
    c = geom->w3_t[cc];
...
/*-------------------------------------------------------------------*/
/* Volume fluxes for semi-Lagrange. The first computation in the     */
/* block of region->dt the fluxes are not computed.                  */
/*-------------------------------------------------------------------*/
void region_volume_flux_trans(geometry_t *window, window_t *windat, win_priv_t *wincon)
{
  ...
  not_implemented("region volume flux");
  hd_quit("Exiting");
...
/*-------------------------------------------------------------------*/
/* Mass fluxes for the transport model (semi-Lagrange).              */
/*-------------------------------------------------------------------*/
		       ...
		       )
{
  ...
  if (window->nregions == 0) return;
  not_implemented("region mass flux");
  hd_quit("Exiting");
