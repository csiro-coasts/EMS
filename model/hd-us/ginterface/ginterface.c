...
/* Only valid for the unstructured model.                            */
void i_get_tracer_e1(void* hmodel, int c, int ntr, int *tmap, double **tr_e1)
{
  geometry_t* window = (geometry_t*) hmodel;
  window_t *windat = window->windat;
  ...
  int dir = 1;
  int c3 = window->nsur_t[cc];
  int xm1 = window->c2c[dir][c3];
  int e = window->c2e[dir][c3];
  int k, n, m;
...
void i_get_tracer_e2(void* hmodel, int c, int ntr, int *tmap, double **tr_e2)
{
  ...
  int bot_k = window->s2k[window->bot_t[cc]];
  int top_k = window->s2k[window->nsur_t[cc]];
  int dir = window->npe[c2];
  int c3 = window->nsur_t[cc];
  int ym1 = window->c2c[dir][c3];
  int e = window->c2e[dir][c3];
  int k, n, m;
  for(k = top_k; k >= bot_k; k--) {
    for(n = 0; n < ntr; n++) {
...
double i_get_sinthcell(void *hmodel, int cc) 
{
  ...
  for (j = 1; j <= window->npe[cs]; j++) {
    e = window->c2e[j][cs];
    v += sin(window->thetau1[e]);
  }
  v /= (double)window->npe[cs];
  return v;
}

/* Returns cell centered cos() of the angle of the e1 normal vector. */
/* Unstructured meshes only.                                         */
double i_get_costhcell(void *hmodel, int cc) 
{
  geometry_t* window = (geometry_t*) hmodel;
...
/* This assumes the flux is retreived from edge 1 of the polygon.    */
/* Unstructured meshes only.                                         */
void i_get_fluxe1_wc(void* hmodel, int c, double *u1flux3d)
{
  geometry_t* window = (geometry_t*) hmodel;
  ...
  int k;
  for(k = top_k; k >= bot_k; k--) {
    u1flux3d[k] = windat->u1flux3d[e];
    e = window->zm1e[e];
  }
}

/* Gets a column of e2 volume fluxes at coordinate c.                */
/* This assumes the flux is retreived from edge 4 of the polygon.    */
/* Unstructured meshes only.                                         */
void i_get_fluxe2_wc(void* hmodel, int c, double *u2flux3d)
{
    geometry_t* window = (geometry_t*) hmodel;
    ...
    int k;
    for(k = top_k; k >= bot_k; k--) {
      u2flux3d[k] = windat->u1flux3d[e];
      e = window->zm1e[e];
    }
}

/* Gets a column of u velocities at coordinate c.                    */
/* Unstructured meshes only.                                         */
void i_get_u1_wc(void* hmodel, int c, double *u1)
{
    geometry_t* window = (geometry_t*) hmodel;
...
/* Gets a column of v velocities at coordinate c.                    */
/* Unstructured meshes only.                                         */
void i_get_u2_wc(void* hmodel, int c, double *u2)
{
    geometry_t* window = (geometry_t*) hmodel;
...
/* u wind stress is in wincon->d2 via vel_cen().                     */
double i_get_wave_wind1(void *hmodel, int c) 
{
  geometry_t* window = (geometry_t*) hmodel;
  win_priv_t *wincon = window->wincon;
  int c2 = window->m2d[c];
  double v = wincon->d2[c2];
  return v;
}
...
/* Returns the north component of cell centered wind stress at       */
/* coordinate c. Unstructured meshes only; assumes the centered      */
/* v wind stress is in wincon->d3 via vel_cen().                     */
double i_get_wave_wind2(void *hmodel, int c) 
{
  geometry_t* window = (geometry_t*) hmodel;
  win_priv_t *wincon = window->wincon;
  int c2 = window->m2d[c];
  double v = wincon->d3[c2];
  return v;
}
