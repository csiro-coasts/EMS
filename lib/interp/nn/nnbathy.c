 ...
 *
 *  File: model/lib/grid/nn/nnbathy.c
 *  
 *  Interpolate scalar 2D data in specified points using
 ...
 *
 *  $Id: nnbathy.c 6597 2020-09-03 05:29:26Z riz008 $
 *
 */
...
int main(int argc, char* argv[])
{
    ...
    points_read(s->fin, 3, &nin, &pin, NULL);
    ...
    if (s->generate_points) {
        ...
        points_read(s->fout, 2, &nout, &pout, NULL);
...
int main(int argc, char* argv[])
{
    ...
    if (s->generate_points) {
        points_getrange(nin, pin, s->zoom, &s->xmin, &s->xmax, &s->ymin, &s->ymax);
        pr = preader_create(s->xmin, s->xmax, s->ymin, s->ymax, s->nx, s->ny);
    } else
        pr = preader_create_from_file(s->fout);
